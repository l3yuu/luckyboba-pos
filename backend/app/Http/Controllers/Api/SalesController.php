...
class SalesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $cashierName = $request->input('cashier_name') ?? ($user ? $user->getAttribute('name') : 'System Admin');
        ...
        $existing = Sale::with('items')
            ...
            ->where('branch_id', auth('sanctum')->user()?->getAttribute('branch_id'))
            ->first();

        if ($existing) {
            return response()->json([
                'status' => 'success',
                'si_number' => $existing->getAttribute('invoice_number'),
        ...
        try {
            ...
            $recalculatedTotal = DB::table('sale_items')
                ->where('sale_id', $sale->getAttribute('id'))
                ->sum('final_price');
    ...
    private function incrementDiscountUsageCount(?int $effectiveDiscountId, ?string $paxDiscountIds, array $items): void
    {
        ...
        if ($effectiveDiscountId) {
            $discountIds->push($effectiveDiscountId);
        }
