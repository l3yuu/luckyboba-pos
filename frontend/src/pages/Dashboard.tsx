  ...
  return (
    <div className="p-5 md:p-7 min-h-full flex flex-col gap-5">
      <div className="grid grid-cols-12 gap-4">
        ...
        {/* Terminal Status — solid #a020f0, same as login purple, NO gradient */}
        <div
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-[#f5f0ff] border border-[#ddd6fe] flex items-center justify-center rounded-lg">
                <DollarSign size={18} className="text-[#a020f0]" strokeWidth={2} />
              </div>
        ...
        <div className="col-span-12 md:col-span-6 lg:col-span-3 bg-white border border-[#e9d5ff] rounded-[0.625rem] p-7 flex flex-col justify-between min-h-44 stat-card">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#f5f0ff] border border-[#ddd6fe] flex items-center justify-center rounded-lg">
              <Receipt size={18} className="text-[#a020f0]" strokeWidth={2} />
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <MetricCard icon={<ArrowDownToLine size={18} strokeWidth={2} className="text-[#a020f0]" />}  label="Cash Out"   value={fmt(stats?.cash_out_today ?? 0)}  isLoading={isLoading} accent="purple" />
      ...
      <div className="grid grid-cols-12 gap-4 flex-1">
        <LeaderboardCard title="Top Sellers Today"  icon={<Trophy size={17} strokeWidth={2} className="text-[#a020f0]" />} sellers={stats?.top_seller_today ?? []}    loading={isLoading} />
        <LeaderboardCard title="All Time Leaders"   icon={<Clock4  size={17} strokeWidth={2} className="text-[#a020f0]" />} sellers={stats?.top_seller_all_time ?? []} loading={isLoading} />
      </div>
    </div>
  );
};
...
const accentMap = { emerald: 'text-emerald-700', purple: 'text-[#a020f0]', red: 'text-red-600' };
                ...
                {item ? (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-bold tabular-nums text-[#a020f0] bg-[#f5f0ff] border border-[#ddd6fe] px-3 py-1 rounded-sm">
                        {item.total_qty} sold
                      </span>
                    </div>
                    <div className="h-0.5 bg-[#ede9fe] overflow-hidden rounded-full">
                      <div className="rank-bar h-full bg-[#a020f0]" style={{ width: `${(item.total_qty / max) * 100}%` }} />
                    </div>
                  </>
                ) : (
                  <div className="flex items-center gap-4">
  ...
  <>
    ...
    <div className="flex h-screen bg-[#f5f0ff] overflow-hidden">
      ...
      <div className="flex-1 p-7 flex flex-col gap-5">
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-5 h-44 animate-pulse rounded-[0.625rem]" style={{ backgroundColor: '#a020f0', opacity: 0.25 }} />
