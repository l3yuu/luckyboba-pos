...
const Dashboard = () => {
  ...
  const goToDashboardFresh = () => {
    ...
    // activeTab effect will fire and fetch automatically
  };

  const refreshStats = () => {
    void (async () => { await fetchStats(true); })();
  };
            ...
            {isInitialLoad ? (
              ...
              <p className={`text-xl md:text-2xl font-black transition-all duration-500 ${
                ...
                {stat.isCurrency === false
                  ? stat.value
                  : `₱${Number(stat.value).toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
              </p>
            )}
          </div>
        ))}
...
const TopSellerList = ({ title, sellers, loading }: { title: string; sellers: TopSeller[] | null; loading: boolean; }) => (
