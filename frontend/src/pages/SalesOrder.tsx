...
=======
>>>>>>> 16c718d5de5a9dce404e3eb57dedc5434b475c13
                    ...
                    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                        <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                            ...
                            <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
                                            ...
                                            <div className="flex gap-2">
                                                {SUGAR_LEVELS.map((level) => (
                                                    <button key={level} onClick={() => setSugarLevel(level)}
                                                        className={`flex-1 py-2 rounded-xl text-[10px] font-black transition-all ${sugarLevel === level ? 'bg-[#3b2063] text-white shadow-md' : 'bg-zinc-50 text-zinc-400 border border-zinc-100 hover:bg-zinc-100'}`}>
                                                        {level}
                                                    </button>
                                                ))}
                                            </div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {EXTRA_OPTIONS.map((opt) => (
                                                    <button key={opt} onClick={() => toggleOption(opt)}
                                                        className={`px-3 py-2 rounded-xl text-[10px] font-bold uppercase transition-all ${selectedOptions.includes(opt) ? 'bg-[#3b2063] text-white shadow-md' : 'bg-zinc-50 text-zinc-400 border border-zinc-100 hover:bg-zinc-100'}`}>
                                                        {opt}
                                                    </button>
                                                ))}
                                            </div>
                                ...
                                {/* ── DELIVERY CHARGE (ORDER-LEVEL) ── */}
                                <div>
                                    <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest ml-2 mb-2 block">
                                        Charges (+10.00)
                                        {orderCharge && (
                                            <span className={`ml-2 px-2 py-0.5 rounded-full text-[9px] font-black ${orderCharge === 'grab' ? 'bg-green-100 text-green-700' : 'bg-pink-100 text-pink-700'}`}>
                                                Order-wide {orderCharge === 'grab' ? 'GrabFood' : 'FoodPanda'} — tap to remove
                                            </span>
                                        )}
                                    </label>
                                    <div className="grid grid-cols-2 gap-3">
                                        <button
                                            type="button"
                                            onClick={() => orderCharge !== 'panda' && toggleOrderCharge('grab')}
                                            disabled={orderCharge === 'panda'}
                                            className={`p-3 rounded-2xl border-2 transition-all flex items-center justify-center gap-2 ${
                                                orderCharge === 'panda' 
                                                ? 'border-zinc-100 bg-zinc-50 text-zinc-300 opacity-40 cursor-not-allowed' 
                                                : orderCharge === 'grab'
                                                    ? 'border-green-500 bg-green-50 text-green-700 ring-2 ring-green-200' 
                                                    : 'border-zinc-100 bg-zinc-50 text-zinc-400 hover:border-green-200'
                                            }`}
                                        >
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => orderCharge !== 'grab' && toggleOrderCharge('panda')}
                                            disabled={orderCharge === 'grab'}
                                            className={`p-3 rounded-2xl border-2 transition-all flex items-center justify-center gap-2 ${
                                                orderCharge === 'grab' 
                                                ? 'border-zinc-100 bg-zinc-50 text-zinc-300 opacity-40 cursor-not-allowed' 
                                                : orderCharge === 'panda'
                                                    ? 'border-pink-500 bg-pink-50 text-pink-700 ring-2 ring-pink-200'
                                                    : 'border-zinc-100 bg-zinc-50 text-zinc-400 hover:border-pink-200'
                                            }`}
                                        >
                    ...
                    <div className="fixed inset-0 z-120 flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
                        <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden">
                            <div className="bg-[#3b2063] p-6 text-white text-center">
                                ...
                                {orderCharge && (
                                    <div className={`mt-2 inline-block px-3 py-1 rounded-full text-[10px] font-black ${orderCharge === 'grab' ? 'bg-green-400/20 text-green-200' : 'bg-pink-400/20 text-pink-200'}`}>
                                        {orderCharge === 'grab' ? 'GrabFood Order (+₱10/item)' : 'FoodPanda Order (+₱10/item)'}
                                    </div>
                                )}
                            </div>
                                        ...
                                        {selectedCategory.menu_items
                                            ...
                                            .map((item) => (
                                                <button key={item.id} onClick={() => handleItemClick(item)}
                                                    className="bg-white hover:bg-[#3b2063] hover:text-white text-[#3b2063] p-4 rounded-2xl shadow-sm border border-zinc-100 h-24 text-xs uppercase font-bold text-center">
                                                    {item.name}
                                                </button>
                                            ))}
                                    </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 pb-20 animate-in fade-in zoom-in duration-300">
                                {filteredCategories.map((cat) => (
                                    <button key={cat.id} onClick={() => handleCategoryClick(cat)}
                                        className="bg-white hover:bg-[#3b2063] hover:text-white text-[#3b2063] font-bold text-[10px] uppercase p-3 rounded-2xl h-24 shadow-sm border border-zinc-100 transition-all">
                                        {cat.name}
                                    </button>
                                ))}
                            </div>
                                        ...
                                        <div key={index} className="flex justify-between items-start gap-2 bg-zinc-50 p-3 rounded-xl border border-zinc-100 group">
                                            ...
                                            <div className="flex flex-col items-end gap-1 shrink-0">
                                                <button type="button" onClick={() => removeFromCart(index)}
                                                    className="p-2.5 rounded-xl text-zinc-400 hover:text-red-500 hover:bg-red-50 transition-colors" aria-label="Remove item">
                        ...
                        <div className="bg-[#3b2063] text-white p-6 rounded-t-4xl">
                            ...
                            <div className="pt-2 flex justify-between items-center mt-2">
                                <button onClick={() => setIsConfirmModalOpen(true)} disabled={cart.length === 0}
                                    className="text-xs font-bold uppercase bg-white text-[#3b2063] px-4 py-2 rounded-full disabled:opacity-50">
                                    Confirm Order
                                </button>
