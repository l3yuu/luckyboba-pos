...
import {
  ...
  type CartItem,
  type Bundle,
  type BundleComponentCustomization,
} from '../types/index'
...
const SalesOrder = () => {
  ...
  const [cashierName, setCashierName] = useState<string>(() => localStorage.getItem('lucky_boba_user_name') ?? 'Admin')
  ...
  const filteredAddOns = addOnsData.filter(a => (isWaffleCategory ? a.category === 'waffle' : a.category !== 'waffle'))
  ...
  useEffect(() => {
    setItemPaxAssignments(prev => {
      ...
      cart.forEach((item, i) => {
        ...
        const arr = Array(item.qty)
          .fill('none')
          .map((_, ui) => existing[ui] ?? 'none') as ('none' | 'sc' | 'pwd')[]
        updated[key] = arr
      })
      return updated
    })
