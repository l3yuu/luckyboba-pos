...
// IMPORT TYPES AND CONSTANTS FROM YOUR TYPES FOLDER
import { 
    type MenuItem, 
    type Category, 
    type CartItem, 
...
useEffect(() => {
    ...
    const handleNavClick = (label: string) => { if (label === 'Home') navigate('/'); };
    ...
    const addToOrder = () => {
        ...
        setCart([...cart, { ...selectedItem, name: isWings ? `${selectedItem.name} (${categorySize})` : selectedItem.name, qty, remarks, charges, sugarLevel: isDrink ? sugarLevel : undefined, size: isDrink ? size : undefined, options: isDrink ? selectedOptions : undefined, addOns: isDrink ? selectedAddOns : undefined, finalPrice: (basePrice + extraCost) * qty }]);
        closeModal();
    };
...
const handleConfirmOrder = async () => {
    ...
    return (
        <>
            <style>
                {` @media print { @page { size: 60mm auto; margin: 0; } html, body { width: 60mm; margin: 0; padding: 0; } body * { visibility: hidden; } .printable-receipt-container, .printable-receipt-container * { visibility: visible; } .printable-receipt-container { position: absolute; left: 0; top: 0; width: 100%; padding: 5mm 4mm; box-sizing: border-box; background: white; color: black; font-family: 'Courier New', Courier, monospace; font-size: 11px; font-weight: 600; line-height: 1.2; } .receipt-header { text-align: center; margin-bottom: 8px; border-bottom: 1px dashed black; padding-bottom: 5px; } .receipt-header h1 { font-size: 16px; margin: 0; font-weight: 800; } .group-header { font-weight: 800; border-bottom: 1px solid black; margin: 8px 0 2px 0; font-size: 10px; text-transform: uppercase; } .item-row { display: flex; justify-content: space-between; margin-bottom: 2px; font-weight: 700; } .modifier-row { display: flex; justify-content: space-between; font-size: 10px; padding-left: 8px; font-weight: normal; } .total-section { border-top: 2px dashed black; margin-top: 10px; padding-top: 5px; font-weight: 900; font-size: 14px; } .footer-text { text-align: center; font-size: 9px; margin-top: 15px; border-top: 1px dashed black; font-style: italic; } } `}
            </style>
            ...
            <div className="flex flex-col h-screen w-screen bg-[#f8f6ff] relative overflow-hidden font-sans print:hidden">
                ...
                <div className="flex gap-4 p-4 border-b border-zinc-200 bg-white items-stretch h-24 shrink-0 shadow-sm z-20">
                    <div className="flex gap-2">
                        {['Home', 'Cat', 'Kit', 'Bar', 'Bill'].map((label) => (
                            <button key={label} onClick={() => handleNavClick(label)} className="bg-[#3b2063] hover:bg-[#2a1647] text-white w-20 h-full rounded-2xl font-bold text-xs uppercase tracking-wider flex flex-col items-center justify-center gap-1 group transition-all active:scale-95">
                                {label}
                            </button>
                        ))}
                    </div>
                ...
                <div className="flex flex-1 overflow-hidden relative z-10">
                        ...
                        {selectedCategory ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center h-full gap-6">
                                        ...
                                        {isWings ? (
                                            <div className="grid grid-cols-2 gap-6 w-full max-w-2xl">
                                                {WINGS_QUANTITIES.map((sz) => (
                                                    <button key={sz} onClick={() => setCategorySize(sz)} className="h-48 bg-white rounded-3xl shadow-lg border-2 border-transparent hover:border-[#3b2063] transition-all flex flex-col items-center justify-center group font-black uppercase text-[#3b2063] text-2xl">{sz}</button>
                                                ))}
                                            </div>
                                        ) : (
                                            <div className="flex gap-6 w-full max-w-lg">
                                                <button onClick={() => setCategorySize('M')} className="flex-1 h-56 bg-white rounded-3xl shadow-lg border-2 border-transparent hover:border-[#3b2063] transition-all flex flex-col items-center justify-center group font-black uppercase text-[#3b2063]"><DrinkIcon className="w-16 h-16 mb-4 opacity-80" /><span>Medium / Sm</span></button>
                                                <button onClick={() => setCategorySize('L')} className="flex-1 h-56 bg-white rounded-3xl shadow-xl border-2 border-transparent hover:border-[#3b2063] hover:scale-105 transition-all flex flex-col items-center justify-center group font-black uppercase text-[#3b2063]"><DrinkIcon className="w-24 h-24 mb-4" /><span>Large / Lg</span></button>
                                            </div>
                                        )}
                                    </div>
                                ) : (
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 pb-20 animate-in fade-in zoom-in duration-300">
                                {filteredCategories.map((cat) => (
                                    <button key={cat.id} onClick={() => handleCategoryClick(cat)} className="bg-white hover:bg-[#3b2063] hover:text-white text-[#3b2063] font-bold text-[10px] uppercase p-3 rounded-2xl h-24 shadow-sm border border-zinc-100 transition-all">{cat.name}</button>
                                ))}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="w-96 bg-white border-l border-zinc-200 flex flex-col shrink-0 shadow-2xl z-30">
                        <div className="bg-zinc-50 border-b border-zinc-200 p-4 text-center"><h2 className="text-[#3b2063] font-black uppercase text-xs">Current Order</h2></div>
                                    ...
                                    {cart.map((item, index) => (
                                        <div key={index} className="flex justify-between items-start gap-2 bg-zinc-50 p-3 rounded-xl border border-zinc-100 group">
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex flex-wrap gap-1 mt-1 uppercase font-bold text-[9px]">
                                                    {item.size && <span className="bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded">{item.size}</span>}{item.sugarLevel && <span className="bg-[#3b2063]/10 text-[#3b2063] px-1.5 py-0.5 rounded">SUGAR {item.sugarLevel}</span>}{item.options?.map(o => <span key={o} className="bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded">{o}</span>)}
                                                </div>
                                            </div>
                                            <div className="text-right shrink-0">
                                                <button onClick={() => removeFromCart(index)} className="p-2.5 rounded-xl text-zinc-400 hover:text-red-500 hover:bg-red-50 transition-colors">
                        ...
                        <div className="bg-[#3b2063] text-white p-6 rounded-t-4xl">
                            ...
                            <div className="pt-2 flex justify-between items-center mt-2">
                                <button onClick={() => setIsConfirmModalOpen(true)} disabled={cart.length === 0} className="text-xs font-bold uppercase bg-white text-[#3b2063] px-4 py-2 rounded-full disabled:opacity-50">Confirm Order</button>
        ...
        {selectedItem && !isAddOnModalOpen && !isConfirmModalOpen && (
          <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            ...
            {/* MODAL: ADD-ONS */}
            {isAddOnModalOpen && (
                <div className="fixed inset-0 z-110 flex items-center justify-center bg-black/50 backdrop-blur-md p-4 animate-in zoom-in">
                    <div className="bg-white w-full max-w-lg rounded-4xl shadow-2xl flex flex-col h-[80vh] overflow-hidden">
                        <div className="bg-[#3b2063] p-6 text-white text-center relative shrink-0"><h2 className="text-lg font-black uppercase tracking-wider">Select Add-ons</h2><button onClick={() => setIsAddOnModalOpen(false)} className="absolute top-6 right-6 text-white font-bold text-xs bg-white/20 px-3 py-1.5 rounded-lg uppercase">Done</button></div>
                        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 grid grid-cols-2 md:grid-cols-3 gap-3">
                            {getAddOnsSinkers().map((addon) => (
                                <button key={addon.id} onClick={() => toggleAddOn(addon.name)} className={`p-3 rounded-xl text-left border-2 transition-all h-24 flex flex-col justify-between ${selectedAddOns.includes(addon.name) ? 'bg-[#3b2063] border-[#3b2063] text-white' : 'bg-white border-zinc-100 text-zinc-500'}`}><span className="text-[10px] font-black uppercase leading-tight line-clamp-2">{addon.name}</span><span className="text-xs font-bold">₱{Number(addon.price).toFixed(2)}</span></button>
                            ))}
                        </div>
                        <div className="p-4 border-t bg-zinc-50 rounded-b-4xl"><button onClick={() => setIsAddOnModalOpen(false)} className="w-full bg-[#3b2063] text-white py-4 rounded-xl font-black uppercase shadow-lg transition-all">Confirm Selection ({selectedAddOns.length})</button></div>
                    </div>
                </div>
            )}
            ...
            {isConfirmModalOpen && (
                <div className="fixed inset-0 z-120 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in zoom-in">
                    <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden">
                        <div className="bg-[#3b2063] p-6 text-white text-center"><h2 className="text-xl font-black uppercase tracking-widest">Confirm Order</h2></div>
                        <div className="p-6 flex-1 overflow-y-auto max-h-[60vh] bg-zinc-50">{cart.length === 0 ? <p className="text-center text-zinc-400 font-bold text-sm py-8">Cart is empty.</p> : (<div className="space-y-4">{cart.map((item, i) => (<div key={i} className="flex justify-between items-start border-b pb-3 mb-3 last:border-0"><div className="flex-1 min-w-0"><p className="font-bold text-sm text-[#3b2063]">{item.qty}x {item.name}</p><div className="text-[10px] text-zinc-500 mt-1 ml-2">{item.size && <p>• Size: {item.size}</p>}{item.sugarLevel && <p>• Sugar: {item.sugarLevel}</p>}{item.options?.map(o => <p key={o}>• {o}</p>)}{item.addOns?.map(a => <p key={a}>• + {a}</p>)}</div></div><p className="font-black text-sm text-[#3b2063]">₱ {item.finalPrice.toFixed(2)}</p></div>))}</div>)}</div>
            ...
            <div className="printable-receipt-container hidden print:block">
                <div className="receipt-header"><h1>LUCKY BOBA</h1><p>Main Branch - QC</p><p>{formatDate(currentDate)} | {formatTime(currentDate)}</p><p>Cashier: ADMIN</p></div>
                <div className="receipt-body">
                    {mediumDrinks.length > 0 && (<div><div className="group-header">Medium Drinks</div>{mediumDrinks.map((item, i) => (<div key={i} className="item-row"><span>{item.qty}x {item.name}</span><span>{item.finalPrice.toFixed(2)}</span></div>))}</div>)}
                    {largeDrinks.length > 0 && (<div><div className="group-header">Large Drinks</div>{largeDrinks.map((item, i) => (<div key={i} className="item-row"><span>{item.qty}x {item.name}</span><span>{item.finalPrice.toFixed(2)}</span></div>))}</div>)}
                    {otherItems.length > 0 && (<div><div className="group-header">Food & Others</div>{otherItems.map((item, i) => (<div key={i} className="item-row"><span>{item.qty}x {item.name}</span><span>{item.finalPrice.toFixed(2)}</span></div>))}</div>)}
                    <div className="total-section"><div className="item-row"><span>TOTAL</span><span>₱ {subtotal.toFixed(2)}</span></div></div>
                    <div className="footer-text">Follow us @LuckyBoba</div>
                </div>
            </div>
        </>
    );
