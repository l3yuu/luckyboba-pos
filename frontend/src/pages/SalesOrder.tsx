...
const SalesOrder = () => {
  ...
  const [sugarLevels, setSugarLevels] = useState<{ id: number; label: string; value: string }[]>(() => {
    try { const c = localStorage.getItem('pos_sugar_levels_cache'); return c ? JSON.parse(c) : []; }
    catch { return []; }
  });
  ...
  const [addOnsData, setAddOnsData] = useState<{
    ...
    grab_price?: number; panda_price?: number; category?: string
  }[]>(() => {
  ...
  const paxSenior = Object.values(itemPaxAssignments).flat().filter(a => a === 'sc').length;
  const paxPwd    = Object.values(itemPaxAssignments).flat().filter(a => a === 'pwd').length;
...
const subtotal = grossSubtotal - itemDiscountTotal;
  ...
  const hasStickers = cart.some(item =>
    ...
    ((orderType === 'take-out' || orderType === 'delivery') &&
      item.sugarLevel === undefined && item.size === 'none' &&
      !item.isBundle && !(item.remarks?.startsWith('[Drink:') ?? false))
  );
  ...
  useEffect(() => {
    try { localStorage.setItem('pos_cart_cache', JSON.stringify(cart)); }
  ...
  const handleItemClick = async (item: MenuItem) => {
    ...
    if (catType === 'mix_and_match') {
      ...
      const newCartItem: CartItem = {
        ...
        size: 'none', finalPrice: Number(item.price),
      };
    ...
    if (isDrinkItem) {
      try {
        ...
        setSugarLevels(levels);
        setSugarLevel('');
      } catch { setSugarLevels([]); setSugarLevel(''); }
    } else {
      setSugarLevels([]);
      setSugarLevel('');
    }
  ...
  const makeToggleOption = (setter: React.Dispatch<React.SetStateAction<string[]>>) => (opt: string) => {
    setter(prev => {
      ...
      if (iceOpts.includes(opt))   { const rest = prev.filter(o => !iceOpts.includes(o));   return prev.includes(opt) ? rest : [...rest, opt]; }
      if (pearlOpts.includes(opt)) { const rest = prev.filter(o => !pearlOpts.includes(o)); return prev.includes(opt) ? rest : [...rest, opt]; }
  ...
  const isSameCartItem = (a: CartItem, b: CartItem): boolean =>
    a.id === b.id && a.size === b.size && a.cupSizeLabel === b.cupSizeLabel &&
    a.sugarLevel === b.sugarLevel && a.remarks === b.remarks &&
    JSON.stringify(a.options ?? []) === JSON.stringify(b.options ?? []) &&
    JSON.stringify(a.addOns  ?? []) === JSON.stringify(b.addOns  ?? []) &&
    a.charges.grab === b.charges.grab && a.charges.panda === b.charges.panda;
    ...
    setCart(prev => {
      ...
      if (idx !== -1) {
        return prev.map((item, i) =>
          i === idx ? { ...item, qty: item.qty + newItem.qty, finalPrice: item.finalPrice + unitPrice * newItem.qty } : item
        );
      }
      return [...prev, newItem];
    });
  ...
  const addToOrder = () => {
    ...
    if (isDrink || isWaffle) {
      selectedAddOns.forEach(name => {
        ...
        if (addon) {
          ...
          if (orderCharge === 'grab' && Number(addon.grab_price ?? 0) > 0) extraCost += Number(addon.grab_price) - baseAddonPrice;
          else if (orderCharge === 'panda' && Number(addon.panda_price ?? 0) > 0) extraCost += Number(addon.panda_price) - baseAddonPrice;
    ...
    if (isCombo) {
      ...
      setComboDrinkSugar(''); setComboDrinkOptions([]); setComboDrinkAddOns([]);
      setSelectedItem(null); setIsAddOnModalOpen(false);
  ...
  const confirmBundleComponent = () => {
    ...
    const customization: BundleComponentCustomization = {
      name: displayName, quantity: component.quantity,
      sugarLevel: bundleComponentSugar, options: bundleComponentOptions, addOns: bundleComponentAddOns,
    };
    ...
    if (bundleComponentIndex < activeBundleItem.items.length - 1) {
      ...
      setBundleComponentSugar(''); setBundleComponentOptions([]); setBundleComponentAddOns([]);
      return;
    }
    ...
    const cartItem: CartItem = {
      id: activeBundleItem.id, category_id: 0,
      ...
      qty: 1, size: 'L', remarks: remarksLines,
      charges: { grab: orderCharge === 'grab', panda: orderCharge === 'panda' },
      finalPrice: Number(activeBundleItem.price) + bundleAddOnCost,
      isBundle: true, bundleId: activeBundleItem.id, bundleComponents: newCustomizations,
    };
  ...
  const confirmComboDrink = () => {
    ...
    comboDrinkAddOns.forEach(name => {
      ...
      if (addon) {
        ...
        else if (orderCharge === 'panda' && Number(addon.panda_price ?? 0) > 0) addOnExtraCost += Number(addon.panda_price);
        else addOnExtraCost += Number(addon.price);
      }
    });
    const drinkDetails = [`Sugar: ${comboDrinkSugar}`, ...comboDrinkOptions, ...comboDrinkAddOns.map(a => `+${a}`)].join(' | ');
    const drinkLabel   = isPizzaCombo && !isClassicPearl ? pendingComboCart.name.replace(/^PIZZA \+ /i, '') : 'Classic Pearl';
    const finalItem: CartItem = {
      ...pendingComboCart,
      remarks:    `${drinkLabel} [${drinkDetails}]${pendingComboCart.remarks ? ` | Note: ${pendingComboCart.remarks}` : ''}`,
      sugarLevel: comboDrinkSugar, options: comboDrinkOptions,
  ...
  const confirmMixAndMatch = () => {
    ...
    const drinkDetails = [`Drink: ${selectedMixMatchDrink.name}`, `Sugar: ${mixMatchDrinkSugar}`, ...mixMatchDrinkOptions, ...mixMatchDrinkAddOns.map(a => `+${a}`)].join(' | ');
    const finalItem: CartItem = {
      ...pendingMixMatchCart,
  ...
  const saveCartItemEdit = () => {
    ...
    const addOnCostPerUnit = (editingCartItem.addOns ?? []).reduce((sum, addonName) => {
      ...
      const price = editingCartItem.charges?.grab && Number(addon.grab_price ?? 0) > 0
        ? Number(addon.grab_price) : editingCartItem.charges?.panda && Number(addon.panda_price ?? 0) > 0
        ? Number(addon.panda_price) : Number(addon.price ?? 0);
      return sum + price;
    }, 0);
    ...
    const updated: CartItem = {
      ...editingCartItem, finalPrice: newFinalPrice, originalPrice: grossTotal,
      discountLabel, discountId: editingItemDiscountId, discountType: itemDiscountType, discountValue: discountValNum,
    };
  ...
  const computeDiscountedTotal = () => {
    ...
    const addOnCostPerUnit = (editingCartItem.addOns ?? []).reduce((sum, addonName) => {
      ...
      if (!a) return sum;
      return sum + (editingCartItem.charges?.grab && Number(a.grab_price ?? 0) > 0 ? Number(a.grab_price) : editingCartItem.charges?.panda && Number(a.panda_price ?? 0) > 0 ? Number(a.panda_price) : Number(a.price));
    }, 0);
    const drinkUnitPrice = Number(editingCartItem.price);
    const qty = editingCartItem.qty;
    const grossTotal = (drinkUnitPrice + addOnCostPerUnit) * qty;
    let totalDiscount = 0;
    if (itemDiscountType === 'percent' && itemDiscountValue !== '') totalDiscount = drinkUnitPrice * qty * (Number(itemDiscountValue) / 100);
    else if (itemDiscountType === 'fixed' && itemDiscountValue !== '') totalDiscount = Math.min(Number(itemDiscountValue), drinkUnitPrice * qty);
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-screen w-screen bg-[#f4f2fb] relative overflow-hidden font-sans print:hidden">
        ...
        {isAddOnModalOpen && (
          <AddOnModalShell title="Select Add-ons" addOns={filteredAddOns} selected={selectedAddOns}
            onToggle={toggleAddOn} onClose={() => setIsAddOnModalOpen(false)} zIndex="z-[110]" orderCharge={orderCharge} />
        )}
        ...
        {isBundleModalOpen && activeBundleItem && (() => {
          ...
          return (
            <BundleModal
              activeBundleItem={activeBundleItem} bundleComponentIndex={bundleComponentIndex}
              bundleComponentSugar={bundleComponentSugar} bundleComponentOptions={bundleComponentOptions}
              bundleComponentAddOns={bundleComponentAddOns} sugarLevels={sugarLevels}
              filteredAddOns={filteredAddOns} bundleComponentAddOnModalOpen={bundleComponentAddOnModalOpen}
              onSugarChange={setBundleComponentSugar} onToggleOption={makeToggleOption(setBundleComponentOptions)}
              onOpenAddOns={() => setBundleComponentAddOnModalOpen(true)}
              onCloseAddOns={() => setBundleComponentAddOnModalOpen(false)}
              onToggleAddOn={name => setBundleComponentAddOns(prev => prev.includes(name) ? prev.filter(a => a !== name) : [...prev, name])}
              onConfirm={confirmBundleComponent}
              onClose={() => { setIsBundleModalOpen(false); setActiveBundleItem(null); }}
              orderCharge={orderCharge} onToggleOrderCharge={toggleBundleOrderCharge}
              bundleGrabPrice={bundleGrabPrice} bundlePandaPrice={bundlePandaPrice}
            />
          );
        })()}
        ...
        {isCombodrinkModalOpen && pendingComboCart && (
          <ComboDrinkModal
            pendingComboCart={pendingComboCart} sugarLevels={sugarLevels}
            comboDrinkSugar={comboDrinkSugar} comboDrinkOptions={comboDrinkOptions}
            comboDrinkAddOns={comboDrinkAddOns} filteredAddOns={filteredAddOns}
            comboDrinkAddOnModalOpen={comboDrinkAddOnModalOpen}
            onSugarChange={setComboDrinkSugar} onToggleOption={toggleComboDrinkOption}
            onOpenAddOns={() => setComboDrinkAddOnModalOpen(true)}
            onCloseAddOns={() => setComboDrinkAddOnModalOpen(false)}
            onToggleAddOn={name => setComboDrinkAddOns(prev => prev.includes(name) ? prev.filter(a => a !== name) : [...prev, name])}
        ...
        {isMixMatchModalOpen && pendingMixMatchCart && (
          <MixAndMatchDrinkModal
            pendingMixMatchCart={pendingMixMatchCart} drinkItems={mixMatchDrinkItems}
            selectedDrink={selectedMixMatchDrink} drinkSugarLevels={sugarLevels}
            drinkSugar={mixMatchDrinkSugar} drinkOptions={mixMatchDrinkOptions}
            drinkAddOns={mixMatchDrinkAddOns} filteredAddOns={filteredAddOns}
            drinkAddOnModalOpen={mixMatchDrinkAddOnOpen} orderCharge={orderCharge}
            onSelectDrink={item => { setSelectedMixMatchDrink(item ?? null); setMixMatchDrinkSugar(''); setMixMatchDrinkOptions([]); }}
            onSugarChange={setMixMatchDrinkSugar} onToggleOption={makeToggleOption(setMixMatchDrinkOptions)}
            onOpenAddOns={() => setMixMatchDrinkAddOnOpen(true)}
            onCloseAddOns={() => setMixMatchDrinkAddOnOpen(false)}
            onToggleAddOn={name => setMixMatchDrinkAddOns(prev => prev.includes(name) ? prev.filter(a => a !== name) : [...prev, name])}
            onToggleOrderCharge={toggleOrderCharge} onConfirm={confirmMixAndMatch}
        ...
        {isConfirmModalOpen && (
          <ConfirmOrderModal
            cart={cart} cashierName={cashierName} totalCount={totalCount}
            subtotal={grossSubtotal} amtDue={amtDue} addOnsData={addOnsData}
            vatableSales={vatableSales} vatAmount={vatAmount} vatExemptSales={vatExemptSales}
            change={change} totalDiscountDisplay={totalDiscountDisplay}
            orderCharge={orderCharge} selectedDiscount={selectedDiscount}
            selectedDiscounts={selectedDiscounts} paymentMethod={paymentMethod}
        ...
        {isCustomerNameModalOpen && (
          <CustomerNameModal
            customerName={customerName} onChange={setCustomerName}
            onSkip={() => { setIsCustomerNameModalOpen(false); handleSubmitOrder(''); }}
            onConfirm={() => { setIsCustomerNameModalOpen(false); handleSubmitOrder(customerName); }}
          />
        )}

        {isSuccessModalOpen && (
          <SuccessModal
            orNumber={orNumber} hasStickers={hasStickers}
            printedReceipt={printedReceipt} printedKitchen={printedKitchen} printedStickers={printedStickers}
            onPrintReceipt={handlePrintReceipt} onPrintKitchen={handlePrintKitchen}
            onPrintStickers={handlePrintStickers} onNewOrder={handleNewOrder}
          />
        )}

        <Header
          branchName={branchName} cashierName={cashierName}
        ...
        <OfflineQueueBanner
          isOnline={isOnline} queue={queue} queueCount={queueCount}
      ...
      {[
        ...
        <div className="flex flex-1 overflow-hidden relative z-10">
          <MenuArea
            ...
            categorySize={categorySize} searchQuery={searchQuery}
            filteredCategories={filteredCategories} isWings={isWings}
            categoryHasOnlyOneSize={categoryHasOnlyOneSize} isDrink={isDrink}
            getFilteredItems={getFilteredItems} onCategoryClick={handleCategoryClick}
            onItemClick={handleItemClick} onSizeSelect={setCategorySize} onBack={handleBack}
          />
          <CartSidebar
            cart={cart} cashierName={cashierName} orNumber={orNumber}
            terminalNumber={terminalNumber} totalCount={totalCount} subtotal={subtotal}
