// frontend/src/pages/SystemReports.tsx
...
function StatCard({ label, value, sub, color = T.purple }: { label: string; value: string; sub?: string; color?: string }) {
  return (
    <div style={{ background: T.purplePale, borderRadius: 12, padding: '16px 18px' }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 800, color, fontFamily: "'DM Mono', monospace", lineHeight: 1.1 }}>{value}</div>
...
function LoadingSpinner() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 0', gap: 14 }}>
      <div style={{
        width: 40, height: 40, border: `3px solid ${T.purplePale}`,
        borderTop: `3px solid ${T.purple}`, borderRadius: '50%',
  ...
  return (
    ...
    >
      <div style={{
        ...
        boxShadow: '0 24px 64px rgba(61,44,141,0.22)',
        ...
        <div style={{
          ...
          padding: '22px 28px', borderBottom: `1.5px solid ${T.border}`,
          background: `linear-gradient(135deg, ${T.purplePale} 0%, #fff 60%)`,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{
              width: 42, height: 42, borderRadius: 12,
              background: T.purple, display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 20, boxShadow: `0 4px 12px ${T.purple}44`,
            }}>{icon}</div>
            <div>
              <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: T.text, fontFamily: "'Nunito', sans-serif" }}>{title}</h2>
              <p  style={{ margin: 0, fontSize: 12, color: T.sub }}>{subtitle}</p>
            </div>
          </div>
          <button onClick={onClose} style={{
            width: 36, height: 36, borderRadius: 10, border: `1.5px solid ${T.border}`,
            background: '#fff', cursor: 'pointer', fontSize: 20, color: T.sub,
...
type SalesAction =
  | { type: 'FETCH_START' }
  | { type: 'FETCH_SUCCESS'; payload: SalesSummaryResponse }
...
function SalesSummaryModal({ onClose }: { onClose: () => void }) {
  ...
  const fetchData = (p: Period) => setPeriod(p);
  ...
  return (
    <ModalShell title="Sales Summary" subtitle="Complete sales report across all branches" icon="📊" onClose={onClose}>
      ...
      {error   && <ErrorBanner message={error} onRetry={() => fetchData(period)} />}
            ...
            <div style={{ background: T.purplePale, borderRadius: 12, padding: '16px 16px 8px' }}>
              ...
              <ResponsiveContainer width="100%" height={160}>
                <AreaChart data={data.breakdown} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="revenue" name="Revenue" stroke={T.purple} strokeWidth={2.5} fill="url(#ssg)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            <div style={{ background: T.purplePale, borderRadius: 12, padding: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {data.revenue_per_branch.map((b, i) => {
                  const pct = (Number(b.total_revenue) / maxBr) * 100;
                  return (
                    <div key={b.branch_id}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                        <span style={{ fontSize: 13, fontWeight: 700, color: T.text }}>{b.branch_name}</span>
                        <span style={{ fontSize: 13, fontWeight: 800, color: BR_COLORS[i] ?? T.purple, fontFamily: "'DM Mono', monospace" }}>{fmtC(b.total_revenue)}</span>
                      </div>
                      <div style={{ height: 6, background: '#ddd9f0', borderRadius: 4, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${pct}%`, background: BR_COLORS[i] ?? T.purple, borderRadius: 4 }} />
                      </div>
              ...
              {data.top_products.slice(0, 5).map((p, i) => {
                ...
                return (
                  <div key={i} style={{ background: '#fff', borderRadius: 10, padding: '12px 14px', border: `1px solid ${i === 0 ? T.purpleMid : T.border}` }}>
                    <div style={{ fontSize: 16, marginBottom: 4 }}>{medals[i]}</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: T.text, lineHeight: 1.3, marginBottom: 6 }}>{p.product_name}</div>
                    <div style={{ fontSize: 18, fontWeight: 800, color: BR_COLORS[i] ?? T.purple, fontFamily: "'DM Mono', monospace" }}>{p.total_quantity.toLocaleString()}</div>
              ...
              <tbody>
                {data.breakdown.map((row, i) => (
                  <tr key={i} style={{ borderTop: `1px solid ${T.border}` }}>
                    <td style={{ padding: '10px', fontSize: 13, color: T.sub,   fontFamily: "'DM Mono', monospace" }}>{row.date}</td>
                    <td style={{ padding: '10px', fontSize: 14, fontWeight: 800, color: T.purple, fontFamily: "'DM Mono', monospace" }}>{fmtC(row.revenue)}</td>
                    <td style={{ padding: '10px', fontSize: 13, color: T.text }}>{Number(row.orders).toLocaleString()}</td>
                    <td style={{ padding: '10px', fontSize: 13, color: T.text,  fontFamily: "'DM Mono', monospace" }}>{fmt(row.avg_order_value)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
/* ═══════════════════════════════════════════════════════════════
   MODAL: Branch Comparison  — LIVE
═══════════════════════════════════════════════════════════════ */
interface BranchState {
  data: BranchComparisonResponse | null;
  loading: boolean;
  error: string | null;
}
type BranchAction =
  | { type: 'FETCH_START' }
  | { type: 'FETCH_SUCCESS'; payload: BranchComparisonResponse }
  | { type: 'FETCH_ERROR'; payload: string };
            ...
            {data.comparison.map((branch, i) => {
              ...
              return (
                <div key={branch.branch_id} style={{
                  background: T.purplePale, borderRadius: 12, padding: '16px',
                  border: `1.5px solid ${i === 0 ? T.purpleMid : T.border}`,
                  ...
                  {/* Revenue */}
                  <div>
                    <div style={{ fontSize: 22, fontWeight: 800, color: ac, fontFamily: "'DM Mono', monospace", marginBottom: 4 }}>{fmtC(branch.total_revenue)}</div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                    {([
                      ...
                      <div key={l} style={{ background: '#fff', borderRadius: 8, padding: '8px 10px' }}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 800, color: T.text, fontFamily: "'DM Mono', monospace" }}>{v}</div>
                      </div>
                    ))}
                  </div>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 3, height: 5, borderRadius: 4, overflow: 'hidden' }}>
                        {branch.payment_methods.map((pm, j) => (
                          <div key={j} style={{ flex: pm.count, background: PAY_COLORS[pm.payment_method] ?? T.purple }}
...
/* ═══════════════════════════════════════════════════════════════
   REPORT CARDS (main page grid)
═══════════════════════════════════════════════════════════════ */
const REPORT_CARDS = [
  { key: 'sales',     icon: '📊', title: 'Sales Summary',    desc: 'Complete sales report across all branches',   live: true  },
  { key: 'inventory', icon: '🗄️', title: 'Inventory Report', desc: 'Stock levels and inventory movement',         live: false },
  { key: 'users',     icon: '👥', title: 'User Activity',    desc: 'Login history and user actions',              live: false },
  { key: 'branch',    icon: '🔀', title: 'Branch Comparison',desc: 'Performance comparison between branches',     live: true  },
  { key: 'financial', icon: '💰', title: 'Financial Report', desc: 'Revenue, expenses, and profit margins',       live: false },
  { key: 'audit',     icon: '📋', title: 'Audit Log',        desc: 'System changes and administrative actions',  live: false },
];

function ReportCard({ card, onGenerate }: { card: typeof REPORT_CARDS[0]; onGenerate: (key: string) => void }) {
  ...
  return (
    <div
      ...
      style={{
        background: T.card, borderRadius: 16, padding: '28px',
        border: `1.5px solid ${hover ? T.purpleMid : T.border}`,
        boxShadow: hover ? `0 8px 24px ${T.purple}18` : '0 2px 8px rgba(0,0,0,0.04)',
        transition: 'all .2s', display: 'flex', flexDirection: 'column', gap: 10,
      }}
    >
      <div style={{ fontSize: 28, lineHeight: 1 }}>{card.icon}</div>
      <div>
        <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: T.text, fontFamily: "'Nunito', sans-serif" }}>{card.title}</h3>
        <p  style={{ margin: '4px 0 0', fontSize: 13, color: T.sub, lineHeight: 1.4 }}>{card.desc}</p>
      </div>
      <button
        onClick={() => onGenerate(card.key)}
        style={{
          marginTop: 'auto', padding: '10px', borderRadius: 10, border: 'none', cursor: 'pointer',
          background: hover && card.live ? T.purple : T.purplePale,
          color:      hover && card.live ? '#fff'   : T.purpleLight,
          fontFamily: "'Nunito', sans-serif", fontWeight: 800, fontSize: 13,
          letterSpacing: '0.06em', textTransform: 'uppercase', transition: 'all .2s',
          boxShadow: hover && card.live ? `0 4px 12px ${T.purple}44` : 'none',
        }}
      >
        Generate
      </button>
...
export default function SystemReports() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, fontFamily: "'Nunito', 'Segoe UI', sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800;900&family=DM+Mono:wght@400;500;600&display=swap');
        * { box-sizing: border-box; }
        @keyframes modalIn { from { opacity:0; transform:translateY(-16px) scale(0.97); } to { opacity:1; transform:none; } }
        @keyframes fadeIn  { from { opacity:0; transform:translateY(8px); } to { opacity:1; transform:none; } }
        @keyframes spin    { to { transform: rotate(360deg); } }
        .reports-page { animation: fadeIn .3s ease both; }
      `}</style>

      <div className="reports-page" style={{ padding: '40px 48px' }}>
        <div style={{ marginBottom: 36 }}>
          <h1 style={{ margin: 0, fontSize: 30, fontWeight: 900, color: T.text, fontFamily: "'Nunito', sans-serif", letterSpacing: '-0.01em' }}>
            SYSTEM REPORTS
          </h1>
          <p style={{ margin: '4px 0 0', fontSize: 12, color: T.sub, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700 }}>
