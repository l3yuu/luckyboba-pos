        ...
        {rows.length === 0 ? <p className="text-[11px] text-center py-2">No receipts found.</p> : (
          <>
            ...
            {rows.map((tx, i) => {
              ...
              const dateTime = (tx as { Date_Time?: string; Date?: string }).Date_Time ?? (tx as { Date?: string }).Date ?? '';
              ...
              return (
                <div key={i} className="border-b border-dotted border-zinc-300 py-0.5">
                  ...
                  <div className="flex text-[10px] leading-snug text-zinc-500">
                    ...
                    <span className={`text-right text-[10px] uppercase ${isCancelled ? 'text-red-400' : 'text-zinc-400'}`}>{status}</span>
                  </div>
                </div>
              );
            })}
    ...
    return (
      <div className="my-2">
        ...
        {(() => {
          ...
          // NET = pre-discount gross - all discounts - VAT
          ...
          return (
            <>
              ...
              <Divider />
              {[
                { label: 'VATABLE SALES:',    value: phCurrency.format((reportData?.is_vat ?? isVat) ? vatableSales : 0) },
                { label: 'VAT AMOUNT:',       value: phCurrency.format((reportData?.is_vat ?? isVat) ? vatAmt : 0) },
  ...
  return (
    <div className="flex-1 bg-[#f4f2fb] h-full flex flex-col overflow-hidden font-sans">
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col relative">
        ...
        <div className="bg-white p-3 border border-zinc-200 mb-6 flex flex-col xl:flex-row items-center gap-3 relative z-50 print:hidden shadow-sm rounded-[0.625rem]">
          ...
          <div className="flex-1 w-full flex gap-2">
            <input
              ...
              className="flex-1 px-4 h-11 border border-zinc-300 bg-[#f5f0ff] font-bold text-sm rounded-[0.625rem] focus:outline-none focus:border-[#a020f0]"
            />
            {reportData?.report_type === 'search' && (
              <div className="flex gap-2 flex-1">
                ...
                <button
                  ...
                  disabled={loading}
                  className="px-5 h-11 bg-[#a020f0] text-white font-bold text-xs uppercase tracking-widest hover:bg-[#6a12b8] transition-colors disabled:opacity-50 rounded-[0.625rem]"
                >
                  Search
                </button>
              </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={loading}
              className="px-6 h-11 bg-[#a020f0] text-white font-bold text-xs uppercase tracking-widest hover:bg-[#6a12b8] active:bg-[#5a0fa0] transition-colors disabled:opacity-50 rounded-[0.625rem] border border-[#a020f0]"
            >
        ...
        {error && (
          <div className="mb-4 p-3 bg-red-50 border border-red-200 text-red-700 text-sm font-bold print:hidden rounded-[0.625rem]">
            ⚠️ {error}
          </div>
        )}
