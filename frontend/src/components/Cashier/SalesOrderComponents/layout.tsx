  ...
  <div className="flex gap-3 px-4 py-3 bg-white border-b border-[#e9d5ff] items-center h-20 shrink-0 shadow-sm z-20">
    ...
    <div className="flex-1 bg-[#f5f0ff] rounded-[0.625rem] border-2 border-[#e9d5ff] flex items-center px-4 gap-2 h-full focus-within:border-[#6a12b8] transition-colors">
      ...
      <input
        ...
        className="flex-1 bg-transparent font-bold text-black outline-none uppercase placeholder:text-[#6a12b8]/30 text-sm"
      />
    </div>
    ...
    <div className={`flex-1 overflow-y-auto p-5 bg-[#f4f2fb] transition-all duration-500 ${!menuAvailable ? 'pointer-events-none select-none' : ''}`}>
      ...
      {!menuAvailable && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-[#f4f2fb]/80 backdrop-blur-sm">
          <div className="w-16 h-16 bg-[#6a12b8]/10 rounded-[0.625rem] flex items-center justify-center mb-4">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 text-[#6a12b8]/40">
              <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" />
            </svg>
          </div>
          <p className="text-[#6a12b8]/40 font-black uppercase text-xs tracking-widest">Complete cash in to unlock menu</p>
        </div>
      )}
      ...
      {selectedCategory ? (
        ...
        <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          <div className="flex items-center gap-3 mb-5 sticky top-0 z-10 bg-[#f4f2fb] py-2">
            <button onClick={onBack} className="bg-white p-3 rounded-[0.625rem] shadow-sm border-2 border-[#e9d5ff] text-[#6a12b8] hover:border-[#6a12b8] hover:bg-[#f5f0ff] transition-all">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
              </svg>
            </button>
            <div>
              <div className="text-[10px] font-bold text-[#6a12b8]/60 uppercase tracking-widest leading-none mb-0.5">Category</div>
          ...
          {categorySize ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 pb-20">
              ...
              {getFilteredItems(selectedCategory.menu_items).length === 0 && (
                <div className="col-span-full text-center text-[#6a12b8]/60 font-bold text-sm py-12 uppercase tracking-widest">
            ...
            <div className="flex flex-col items-center justify-center flex-1 gap-5">
              <div className="text-center">
                <div className="text-[10px] font-bold text-[#6a12b8]/60 uppercase tracking-widest mb-1">Step</div>
                <h3 className="text-xl font-black text-[#6a12b8] uppercase">{isWings ? 'Select Quantity' : 'Select Size'}</h3>
              </div>
                ...
                <div className="grid grid-cols-2 gap-4 w-full max-w-2xl">
                  {WINGS_QUANTITIES.map((qty: string) => (
                    <button key={qty} onClick={() => onSizeSelect(qty)}
                      className="h-44 bg-white rounded-[0.625rem] shadow-md border-2 border-[#e9d5ff] hover:border-[#6a12b8] hover:shadow-xl hover:scale-105 transition-all flex flex-col items-center justify-center font-black uppercase text-sm text-black">
                      {qty}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-5 w-full max-w-md flex-wrap justify-center">
                  {selectedCategory.sub_categories.map(sub => (
                    <button key={sub.id} onClick={() => onSizeSelect(sub.name)}
                      className="flex-1 min-w-35 h-56 bg-white rounded-[0.625rem] shadow-md border-2 border-[#e9d5ff] hover:border-[#6a12b8] hover:shadow-xl hover:scale-105 transition-all flex flex-col items-center justify-center font-black text-sm text-black">
                      <DrinkIcon className="w-14 h-14 mb-3 opacity-70" />
                      <span className="text-3xl font-black tracking-widest">{sub.name}</span>
                      <span className="mt-2 bg-[#6a12b8]/10 text-[#6a12b8] text-sm font-black px-3 py-1 rounded-full tracking-widest">
                ...
                <div className="flex gap-5 w-full max-w-md">
                  {[
                    ...
                    <button key={key} onClick={() => onSizeSelect(key)}
                      className="flex-1 h-56 bg-white rounded-[0.625rem] shadow-md border-2 border-[#e9d5ff] hover:border-[#6a12b8] hover:shadow-xl hover:scale-105 transition-all flex flex-col items-center justify-center font-black text-sm text-black">
                      <DrinkIcon className={`${sizeClass} mb-3 opacity-70`} />
                      <span className="text-3xl font-black tracking-widest">{key}</span>
                      <span className="mt-2 bg-[#6a12b8]/10 text-black text-sm font-black px-3 py-1 rounded-full tracking-widest">{label}</span>
                    </button>
                  ))}
                </div>
              )}
        ...
        <div className="pb-20 animate-in fade-in zoom-in duration-300 space-y-6">
          {(() => {
            ...
            if (totalCount === 0) return (
              <div className="text-center text-[#6a12b8]/60 font-bold text-sm py-12 uppercase tracking-widest">
            ...
            return (
              <>
                <div className="flex items-center gap-3 mb-2 px-1">
                  <span className="bg-[#6a12b8] text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full shadow-sm">
                    Search Results
                  </span>
                  <div className="flex-1 h-px bg-zinc-300/60" />
                  <span className="text-[11px] text-[#6a12b8]/60 font-bold">{totalCount} item{totalCount !== 1 ? 's' : ''}</span>
                </div>
                  ...
                  return (
                    <div key={cat.id}>
                      ...
                      <div className="flex items-center gap-2 mb-2 px-1">
                        <span className="text-[10px] font-black uppercase tracking-widest text-[#6a12b8] bg-[#6a12b8]/10 px-2.5 py-1 rounded-full">
                          {cat.name}
                        </span>
            ...
            return (
              <div key={colorKey}>
                <div className="flex items-center gap-3 mb-3 px-1">
                  ...
                  <div className="flex-1 h-px bg-zinc-300/60" />
                  <span className="text-[11px] text-[#6a12b8]/60 font-bold">{groupCats.length} categories</span>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                  {groupCats.map(cat => (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                  {others.map(cat => (
                    <button key={cat.id} onClick={() => onCategoryClick(cat)}
                      className={`${BASE_CARD} hover:bg-[#6a12b8] hover:border-[#6a12b8] hover:text-white`}>
                      {cat.name}
                    </button>
                  ))}
                </div>
...
<div className="bg-[#6a12b8] p-4 text-white flex items-center justify-between shrink-0">
  ...
  <div className="text-right">
        ...
        <div className="space-y-2">
          {cart.map((item, index) => (
            <div
              ...
              className="flex justify-between items-start gap-2 bg-[#f5f0ff] p-3 rounded-[0.625rem] border-2 border-[#e9d5ff] hover:border-[#6a12b8]/30 hover:bg-white transition-colors group cursor-pointer"
            >
              ...
              <div className="flex flex-col items-end gap-1.5 shrink-0">
                <span className="text-[13px] font-black text-[#6a12b8]">₱ {Number(item.finalPrice).toFixed(2)}</span>
    ...
    {/* Cart footer */}
    <div className="bg-[#6a12b8] text-white p-5 shrink-0">
      ...
      <button
        ...
        className="w-full py-4 bg-white text-[#6a12b8] font-black uppercase tracking-widest text-sm rounded-[0.625rem] shadow-md disabled:opacity-30 disabled:cursor-not-allowed hover:bg-[#f5f0ff] transition-colors"
      >
