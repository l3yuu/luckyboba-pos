...
export const Header = ({
  ...
  <div className="flex gap-3 px-4 py-3 bg-white border-b border-[#e9d5ff] items-center h-20 shrink-0 shadow-sm z-20">
    <button
      onClick={onHomeClick}
      className="bg-[#3b2063] text-white h-full px-5 rounded-[0.625rem] font-black text-[11px] uppercase tracking-widest shadow-md hover:bg-[#6a12b8] transition-all flex items-center gap-2"
    >
    ...
    <div className="flex-1 bg-[#f5f0ff] rounded-[0.625rem] border-2 border-[#e9d5ff] flex items-center px-4 gap-2 h-full focus-within:border-[#3b2063] transition-colors">
      ...
      {selectedCategory ? (
        ...
        <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          <div className="flex items-center gap-3 mb-5 sticky top-0 z-10 bg-[#f4f2fb] py-2">
            <button onClick={onBack} className="bg-white p-3 rounded-[0.625rem] shadow-sm border-2 border-[#e9d5ff] text-[#3b2063] hover:border-[#3b2063] hover:bg-[#f5f0ff] transition-all">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
              </svg>
            </button>
            <div>
              <div className="text-[10px] font-bold text-[#3b2063]/60 uppercase tracking-widest leading-none mb-0.5">Category</div>
          ...
          {categorySize ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 pb-20">
              ...
              {getFilteredItems(selectedCategory.menu_items).length === 0 && (
                <div className="col-span-full text-center text-[#3b2063]/60 font-bold text-sm py-12 uppercase tracking-widest">
            ...
            <div className="flex flex-col items-center justify-center flex-1 gap-5">
              <div className="text-center">
                <div className="text-[10px] font-bold text-[#3b2063]/60 uppercase tracking-widest mb-1">Step</div>
                ...
                <div className="grid grid-cols-2 gap-4 w-full max-w-2xl">
                  {WINGS_QUANTITIES.map((qty: string) => (
                    <button key={qty} onClick={() => onSizeSelect(qty)}
                      className="h-44 bg-white rounded-[0.625rem] shadow-md border-2 border-[#e9d5ff] hover:border-[#3b2063] hover:shadow-xl hover:scale-105 transition-all flex flex-col items-center justify-center font-black uppercase text-sm text-black">
                      {qty}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-5 w-full max-w-md flex-wrap justify-center">
                  {selectedCategory.sub_categories.map(sub => (
                    <button key={sub.id} onClick={() => onSizeSelect(sub.name)}
                      className="flex-1 min-w-35 h-56 bg-white rounded-[0.625rem] shadow-md border-2 border-[#e9d5ff] hover:border-[#3b2063] hover:shadow-xl hover:scale-105 transition-all flex flex-col items-center justify-center font-black text-sm text-black">
                ...
                <div className="flex gap-5 w-full max-w-md">
                  {[
                    ...
                    <button key={key} onClick={() => onSizeSelect(key)}
                      className="flex-1 h-56 bg-white rounded-[0.625rem] shadow-md border-2 border-[#e9d5ff] hover:border-[#3b2063] hover:shadow-xl hover:scale-105 transition-all flex flex-col items-center justify-center font-black text-sm text-black">
                      <DrinkIcon className={`${sizeClass} mb-3 opacity-70`} />
                      <span className="text-3xl font-black tracking-widest">{key}</span>
                      <span className="mt-2 bg-[#3b2063]/10 text-black text-sm font-black px-3 py-1 rounded-full tracking-widest">{label}</span>
                    </button>
                  ))}
                </div>
              )}
        ...
        <div className="pb-20 animate-in fade-in zoom-in duration-300 space-y-6">
          {(() => {
            ...
            if (totalCount === 0) return (
              <div className="text-center text-[#3b2063]/60 font-bold text-sm py-12 uppercase tracking-widest">
            ...
            return (
              <>
                <div className="flex items-center gap-3 mb-2 px-1">
                  <span className="bg-[#3b2063] text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full shadow-sm">
                    Search Results
                  </span>
                  <div className="flex-1 h-px bg-zinc-300/60" />
                  <span className="text-[11px] text-[#3b2063]/60 font-bold">{totalCount} item{totalCount !== 1 ? 's' : ''}</span>
                </div>
                  ...
                  return (
                    <div key={cat.id}>
                      ...
                      <div className="flex items-center gap-2 mb-2 px-1">
                        <span className="text-[10px] font-black uppercase tracking-widest text-[#3b2063] bg-[#3b2063]/10 px-2.5 py-1 rounded-full">
                          {cat.name}
                        </span>
            ...
            return (
              <div key={colorKey}>
                <div className="flex items-center gap-3 mb-3 px-1">
                  ...
                  <div className="flex-1 h-px bg-zinc-300/60" />
                  <span className="text-[11px] text-[#3b2063]/60 font-bold">{groupCats.length} categories</span>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                  {groupCats.map(cat => (
  ...
  <div className="text-right">
        ...
        <div className="space-y-2">
          {cart.map((item, index) => (
            <div
              ...
              className="flex justify-between items-start gap-2 bg-[#f5f0ff] p-3 rounded-[0.625rem] border-2 border-[#e9d5ff] hover:border-[#3b2063]/30 hover:bg-white transition-colors group cursor-pointer"
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-1 mt-1.5">
                  {item.sugarLevel != null && <span className="bg-[#3b2063]/10 text-black text-[9px] px-1.5 py-0.5 rounded-lg font-bold">🍬 {item.sugarLevel}</span>}
    ...
    <div className="bg-[#3b2063] text-white p-5 shrink-0">
      ...
      <button
        ...
        className="w-full py-4 bg-white text-[#3b2063] font-black uppercase tracking-widest text-sm rounded-[0.625rem] shadow-md disabled:opacity-30 disabled:cursor-not-allowed hover:bg-[#f5f0ff] transition-colors"
      >
