...
import {
  ...
  ArrowRightIcon, ChevronRight, PrinterIcon,
...
interface VisualPrintPreviewModalProps {
...
export const VisualPrintPreviewModal = ({
  type, content, onPrint, onClose
}: VisualPrintPreviewModalProps) => {
  const [printers, setPrinters] = React.useState<{ name: string; isDefault: boolean }[]>([]);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-[#202124] w-full max-w-6xl h-[92vh] rounded-2xl shadow-[0_0_100px_rgba(0,0,0,0.5)] flex overflow-hidden border border-white/5">
        
        {/* Left: Preview Pane (Chrome Gray) */}
        <div className="flex-1 bg-[#525659] overflow-y-auto relative custom-scrollbar flex justify-center p-12">
          <div className="relative shadow-[0_10px_30px_rgba(0,0,0,0.4)]">
            {/* White Paper */}
            <div className="bg-white min-h-[400px] relative origin-top transform scale-100 transition-transform">
               {/* Top Edge */}
              <div className="absolute -top-1 inset-x-0 h-1 bg-white" style={{ clipPath: 'polygon(0% 0%, 5% 100%, 10% 0%, 15% 100%, 20% 0%, 25% 100%, 30% 0%, 35% 100%, 40% 0%, 45% 100%, 50% 0%, 55% 100%, 60% 0%, 65% 100%, 70% 0%, 75% 100%, 80% 0%, 85% 100%, 90% 0%, 95% 100%, 100% 0%)' }} />
              ...
              {/* Bottom Edge */}
              <div className="absolute -bottom-1 inset-x-0 h-1 bg-white" style={{ clipPath: 'polygon(0% 100%, 5% 0%, 10% 100%, 15% 0%, 20% 100%, 25% 0%, 30% 100%, 35% 0%, 40% 100%, 45% 0%, 50% 100%, 55% 0%, 60% 100%, 65% 0%, 70% 100%, 75% 0%, 80% 100%, 85% 0%, 90% 100%, 95% 0%, 100% 100%)' }} />
            </div>
          </div>

          {/* Page Info Overlay */}
          <div className="absolute bottom-6 left-6 bg-black/40 backdrop-blur-md px-4 py-2 rounded-lg border border-white/10 text-[10px] font-black text-white/70 uppercase tracking-widest">
            1 Sheet of Paper
          </div>
        </div>

        {/* Right: Sidebar (Chrome Dark) */}
        <div className="w-[320px] bg-[#202124] flex flex-col border-l border-white/5">
          <div className="p-8">
            <h1 className="text-xl font-medium text-white mb-1">Print</h1>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#8ab4f8] mb-8">
              {type === 'receipt' ? 'Customer Receipt' : type === 'kitchen' ? 'Kitchen Ticket' : 'Drink Stickers'}
            </p>

            <div className="space-y-6">
              {/* Destination */}
              <div>
                <label className="block text-[11px] font-medium text-zinc-400 mb-2">Destination</label>
                <div className="relative">
                  ...
                  <select 
                    ...
                    className="w-full bg-[#35363a] text-zinc-200 text-xs rounded-lg pl-10 pr-10 py-2.5 outline-none border border-transparent focus:border-[#8ab4f8] appearance-none cursor-pointer transition-all"
                  >
                    {printers.length > 0 ? (
                      printers.map(p => (
              ...
              {/* Pages */}
              <div>
                <label className="block text-[11px] font-medium text-zinc-400 mb-2">Pages</label>
                <select className="w-full bg-[#35363a] text-zinc-200 text-xs rounded-lg px-3 py-2.5 outline-none border border-transparent appearance-none">
                  <option>All</option>
              ...
              {/* Copies */}
              <div>
                <label className="block text-[11px] font-medium text-zinc-400 mb-2">Copies</label>
                <input type="number" defaultValue="1" className="w-full bg-[#35363a] text-zinc-200 text-xs rounded-lg px-3 py-2.5 outline-none border border-transparent focus:border-[#8ab4f8] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
              </div>

              {/* Layout */}
              <div>
                <label className="block text-[11px] font-medium text-zinc-400 mb-2">Layout</label>
                <select className="w-full bg-[#35363a] text-zinc-200 text-xs rounded-lg px-3 py-2.5 outline-none border border-transparent appearance-none">
          ...
          {/* Sidebar Footer */}
          <div className="mt-auto p-8 flex items-center justify-end gap-3 bg-[#202124]">
            <button 
              onClick={onClose}
              className="px-6 py-2 rounded-full border border-zinc-700 text-zinc-300 text-xs font-medium hover:bg-zinc-800 transition-colors"
            >
              Cancel
            </button>
            <button 
              onClick={() => onPrint(selectedPrinter)}
              className="px-8 py-2 rounded-full bg-[#8ab4f8] text-[#202124] text-xs font-black hover:bg-[#aecbff] transition-all active:scale-95 shadow-lg shadow-[#8ab4f8]/10"
            >
              Print
            </button>
          </div>
