          ...
          <div className="bg-white w-full flex-1 border border-[#e9d5ff] flex flex-col h-full shadow-sm rounded-[0.625rem]">
                  ...
                  return (
                    <div key={denom} className="grid grid-cols-12 gap-3 items-center px-6 py-3 transition-colors hover:bg-[#f5f0ff] focus-within:bg-[#f4f2fb]">
                      <div className="col-span-4 flex items-center gap-2">
                        <Banknote size={15} className="text-[#6a12b8] shrink-0" />
                      ...
                      <div className="col-span-4">
                        <input
                          ...
                          className={`w-full text-center font-bold text-sm py-2 border outline-none transition-all ${
                            isEodLocked
                              ? 'bg-[#f5f0ff] border-[#e9d5ff] cursor-not-allowed text-[#6a12b8]/50'
                              : 'bg-white border-[#e9d5ff] focus:border-[#6a12b8]'
                          }`}
                        />
                      </div>
                      <div className={`col-span-4 text-right font-bold text-sm tabular-nums ${rowTotal > 0 ? 'text-[#6a12b8]' : 'text-[#6a12b8]/30'}`}>
            ...
            <div className="p-6 border-t border-[#e9d5ff] space-y-4 bg-white">
              <div className="flex items-center justify-between bg-[#f5f0ff] border border-[#e9d5ff] px-5 py-4">
                ...
                <p className={`text-2xl font-bold tabular-nums ${grandTotal > 0 ? 'text-[#6a12b8]' : 'text-[#6a12b8]/30'}`}>
                  ₱{grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </p>
              </div>

              <div className="relative">
                <MessageSquare size={14} className="absolute left-4 top-3.5 text-[#6a12b8]" />
                <textarea
                  value={remarks}
                  ...
                  className={`w-full pl-10 pr-4 py-3 border outline-none text-sm font-medium resize-none h-14 transition-all ${
                    isEodLocked ? 'bg-[#f5f0ff] border-[#e9d5ff] text-[#6a12b8]/50' : 'bg-white border-[#e9d5ff] focus:border-[#6a12b8]'
                  }`}
                />
              </div>
          ...
          <div className="w-full flex-1 bg-white border border-[#e9d5ff] overflow-hidden flex flex-col shadow-sm rounded-[0.625rem]" style={{ minHeight: 0 }}>
            <div className="px-6 py-5 border-b border-[#e9d5ff] flex items-center gap-3">
              <div className="w-9 h-9 bg-[#f5f0ff] border border-[#e9d5ff] flex items-center justify-center">
                <HistoryIcon size={17} className="text-[#6a12b8]" />
              </div>
              <div>
                    ...
                    <tr key={tx.id} className="hover:bg-[#f5f0ff] transition-colors">
                      <td className="px-6 py-3.5">
                        <div className="flex items-center gap-2">
                          <Clock size={13} className="text-[#6a12b8] shrink-0" />
                          <span className="text-sm font-bold text-black tabular-nums">{tx.time}</span>
                        </div>
                        {tx.remarks !== '-' && (
                          <p className="text-[11px] text-[#6a12b8]/60 font-medium mt-0.5 ml-5 truncate max-w-40">{tx.remarks}</p>
                        )}
                      </td>
                      <td className="px-6 py-3.5 text-right">
                        <span className="text-sm font-bold text-[#6a12b8] tabular-nums">
                      ...
                      <td className="px-6 py-3.5 text-center">
                        <button
                          ...
                          className="w-9 h-9 inline-flex items-center justify-center bg-[#f5f0ff] border border-[#e9d5ff] text-[#6a12b8] hover:bg-[#6a12b8] hover:text-white hover:border-[#6a12b8] transition-all rounded-[0.625rem]"
                        >
                  ...
                  {transactions.length === 0 && (
                    <tr>
                      <td colSpan={3} className="px-6 py-20 text-center">
                        <HistoryIcon size={28} className="mx-auto text-[#6a12b8]/30 mb-3" />
                        <p className="text-[11px] font-bold uppercase tracking-widest text-[#6a12b8]/30">No drops recorded today</p>
                      </td>
                    </tr>
                  )}
                </tbody>
