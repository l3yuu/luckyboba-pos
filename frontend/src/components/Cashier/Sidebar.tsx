      ...
      <div className="fixed inset-0 z-200 flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm">
        ...
        >
          ...
          <div className="flex flex-col w-full gap-2">
            <button
              onClick={action}
              className={`w-full py-3 text-[10px] font-bold tracking-[0.18em] uppercase text-white transition-all rounded-[0.625rem] active:scale-[0.98] ${danger ? 'bg-[#be2525] hover:bg-[#a11f1f]' : 'bg-[#a020f0] hover:bg-[#6b11b8]'}`}
            >
              {btnText}
            </button>
            {cancel && (
    ...
    <>
      <style>{`
        ...
        .sb-btn.active::before {
          ...
          width: 3px; background: #a020f0; border-radius: 0 2px 2px 0;
        }
        ...
        .sb-sub:hover { background: #f0e8ff; color: #a020f0; padding-left: 14px; }
        .sb-sub.active { background: #ede9fe; color: #a020f0; font-weight: 700; padding-left: 14px; }
      ...
      {/* Modals */}
      <Modal show={showCashInRequired} icon={<Lock size={19} className="text-[#a020f0]" />}
      ...
      <aside className={`sb-root relative inset-y-0 left-0 z-50 w-64 bg-white border-r border-zinc-200 transform transition-transform duration-300 md:relative md:translate-x-0 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 sb-scroll min-h-0">
          ...
          <div className="px-5 pt-7 pb-6 flex flex-col items-center border-b border-zinc-100">
            ...
            <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#a020f0] rounded-md">
                ...
                <div key={dd.id}>
                  <button onClick={() => toggle(dd.id)} className={`sb-dh ${groupActive ? 'active' : ''}`}>
                    <div className="flex items-center gap-2.5">
                      <span className={groupActive || dd.state ? 'text-[#a020f0]' : 'text-zinc-400'}>{dd.icon}</span>
        ...
        <div className="shrink-0 px-4 py-4 bg-white border-t border-zinc-100">
          {user && (
            <div className="flex items-center gap-2.5 px-3 py-2.5 mb-3 bg-[#f5f0ff] border border-[#e9d5ff] rounded-[0.625rem]">
              <div className="w-7 h-7 rounded-full bg-[#a020f0] flex items-center justify-center shrink-0">
              ...
              <div className="flex flex-col min-w-0">
                ...
                <span className="text-[9px] font-semibold uppercase tracking-[0.12em] text-[#a020f0] leading-tight">{user.role}</span>
              </div>
            </div>
          )}
          <button
...
const NavBtn = ({ active, icon, label, onClick }: NavBtnProps) => (
  <button onClick={onClick} className={`sb-btn ${active ? 'active' : ''}`}>
    <span className={active ? 'text-[#a020f0]' : 'text-zinc-400'}>{icon}</span>
