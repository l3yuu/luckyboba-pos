...
const inputCls = (err?: string) =>
  `w-full text-sm font-medium text-zinc-700 bg-white border rounded-[0.625rem] px-4 py-3 outline-none focus:ring-2 focus:ring-[#6a12b8]/10 focus:border-[#6a12b8] transition-all ${err ? 'border-red-300 bg-red-50' : 'border-[#e9d5ff]'}`;
...
const Field: React.FC<{ label: string; required?: boolean; error?: string; children: React.ReactNode }> = ({ label, required, error, children }) => (
  <div className="space-y-1.5">
    <label className="text-[10px] font-bold uppercase tracking-widest text-[#6a12b8]/60 block">
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm">
      ...
      <div className="relative bg-[#fcfaff] w-full max-w-lg border border-[#e9d5ff] rounded-[0.625rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200" style={dashboardFont}>
        ...
        {/* Header */}
        <div className="flex items-center justify-between px-7 py-5 bg-[#6a12b8] rounded-t-[0.625rem]">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-7 py-6 flex flex-col gap-5">
          ...
          <div className="p-4 bg-white border border-[#e9d5ff] rounded-[0.625rem] flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] font-black uppercase text-[#6a12b8]">Active Status</p>
            ...
            <button
               ...
               className={`w-12 h-6 rounded-full transition-all flex items-center p-1 ${form.is_active ? 'bg-[#6a12b8]' : 'bg-zinc-300'}`}
            >
          ...
          <div className="bg-[#f5f0ff] border border-[#e9d5ff] rounded-[0.625rem] p-5">
            <div className="flex items-center justify-between mb-3 text-[10px] font-bold uppercase tracking-widest">
              <span className="text-[#6a12b8]">Items Provided</span>
            ...
            <div className="relative mb-3">
              <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6a12b8]/40" />
              <input value={matSearch} onChange={e => setMatSearch(e.target.value)}
                className="w-full bg-white border border-[#e9d5ff] rounded-[0.625rem] pl-9 pr-4 py-2 text-xs font-semibold outline-none focus:border-[#6a12b8]"
              ...
              {filteredMats.length === 0 ? (
                ...
                return (
                  <button key={m.id} type="button" onClick={() => toggleMaterial(m.id)}
                    className={`flex items-center gap-3 px-3 py-2 rounded-[0.625rem] border transition-all text-left ${checked ? 'bg-white border-[#6a12b8] shadow-sm' : 'bg-transparent border-transparent hover:bg-white/50'}`}>
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ${checked ? 'border-[#6a12b8] bg-[#6a12b8]' : 'border-zinc-300'}`}>
                      {checked && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-[#6a12b8] truncate">{m.name}</p>
                    </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button type="submit" disabled={saving}
              className="flex-1 py-3 bg-[#6a12b8] hover:bg-[#6a12b8] text-white rounded-[0.625rem] font-black text-[11px] uppercase tracking-widest transition-all disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      <div className="relative bg-[#fcfaff] w-full max-w-sm border border-[#e9d5ff] rounded-[0.625rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" style={dashboardFont}>
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-[#f5f0ff] border border-[#e9d5ff] rounded-[0.625rem] flex items-center justify-center shadow-sm">
                <Building2 size={20} className="text-[#6a12b8]" />
              </div>
              <div>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={row.label} className="flex items-start gap-4 p-3 bg-white border border-[#e9d5ff]/50 rounded-[0.625rem]">
                <div className="w-8 h-8 bg-zinc-50 border border-zinc-100 rounded-lg flex items-center justify-center shrink-0 text-[#6a12b8]/40">
                  {row.icon}
                </div>
                <div>
                  <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-400">{row.label}</p>
                  <p className="text-xs font-black text-[#6a12b8] mt-0.5">{row.value}</p>
                </div>
              </div>
            ))}
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {supplier.materials.map(m => (
                    <span key={m.id} className="text-[10px] font-black bg-[#f5f0ff] text-[#6a12b8] border border-[#e9d5ff] px-3 py-1 rounded-full uppercase tracking-tight">
                      {m.name}
                    </span>
                  ))}
                </div>
        ...
        <div className="px-7 pb-7">
          <button onClick={onClose}
            className="w-full py-3 bg-[#6a12b8] text-white rounded-[0.625rem] font-black text-[11px] uppercase tracking-widest hover:bg-[#6a12b8] transition-all shadow-sm">
            Confirm & Close
          </button>
        </div>
      </div>
    ...
    <div className="flex-1 bg-[#f4f2fb] h-full flex flex-col overflow-hidden font-sans" style={dashboardFont}>
      ...
      <div className="flex-1 p-5 md:p-8 overflow-y-auto flex flex-col gap-6">
        ...
        <div className="flex items-end justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#6a12b8]/50">Inventory Logistics</p>
          ...
          <div className="flex items-center gap-3">
            <button onClick={fetchSuppliers} disabled={loading}
              className="w-11 h-11 bg-white border border-[#e9d5ff] text-[#6a12b8]/60 hover:text-[#6a12b8] hover:border-[#6a12b8] focus:ring-2 focus:ring-[#6a12b8]/10 flex items-center justify-center rounded-[0.625rem] transition-all shadow-sm">
              <RefreshCw size={18} className={loading ? 'animate-spin' : ''} />
            </button>
            <button onClick={() => setAddOpen(true)}
              className="h-11 px-8 bg-[#6a12b8] hover:bg-[#6a12b8] text-white font-black text-[11px] uppercase tracking-[0.2em] transition-all rounded-[0.625rem] shadow-md flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Partners', value: suppliers.length, icon: <Building2 />, accent: '#6a12b8' },
            ...
            <div key={s.label} className="bg-white border border-[#e9d5ff] rounded-[0.75rem] p-5 shadow-sm relative overflow-hidden group hover:border-[#6a12b8] transition-all">
              <div className="absolute -right-2 -top-2 text-[#6a12b8]/5 group-hover:text-[#6a12b8]/10 transition-colors transform rotate-12 scale-150">
                {s.icon}
              </div>
        ...
        <div className="bg-white border border-[#e9d5ff] rounded-[0.75rem] shadow-sm flex flex-col overflow-hidden min-h-[400px]">
          ...
          <div className="p-4 border-b border-zinc-100 flex flex-wrap items-center justify-between gap-4 bg-zinc-50/50">
            <div className="flex items-center gap-3 bg-white border border-[#e9d5ff] rounded-[0.625rem] px-4 py-2 flex-1 min-w-[280px] focus-within:border-[#6a12b8] focus-within:ring-2 focus-within:ring-[#6a12b8]/5 transition-all">
              <Search size={16} className="text-[#6a12b8]/30" />
            ...
            <div className="flex items-center gap-3">
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                className="bg-white border border-[#e9d5ff] rounded-[0.625rem] px-4 py-2 text-[10px] font-black text-[#6a12b8] uppercase tracking-widest outline-none focus:border-[#6a12b8] h-10 shadow-sm cursor-pointer">
                <option value="">Status: All</option>
                <option value="active">Status: Active Only</option>
                <option value="inactive">Status: Suspended</option>
              </select>
              <span className="text-[10px] font-black text-[#6a12b8]/40 uppercase tracking-widest hidden sm:block">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-[#fcfaff] border-b border-[#e9d5ff]">
                  {['Supplier Identity', 'Lead Contact', 'Item Portfolio', 'System Status', 'Control'].map(h => (
                    <th key={h} className="px-7 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-[#6a12b8]/40">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-50">
                {loading ? (
                  ...
                  <tr key={s.id} className="group hover:bg-[#fcfaff] transition-all border-l-4 border-l-transparent hover:border-l-[#6a12b8]">
                    <td className="px-7 py-4">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-[#f5f0ff] border border-[#e9d5ff] rounded-[0.625rem] flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform shadow-sm">
                          <Building2 size={16} className="text-[#6a12b8] opacity-60" />
                        </div>
                        <div>
                    ...
                    <td className="px-7 py-4">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-[#6a12b8] bg-[#f5f0ff] px-2.5 py-1 rounded-[0.5rem] border border-[#e9d5ff] shadow-sm">
                    ...
                    <td className="px-7 py-4">
                      <div className="flex items-center gap-2">
                        {[
                          { icon: <Search size={14} />,   act: () => setViewTarget(s), color: 'hover:bg-[#6a12b8] hover:text-white', tip: 'Audit' },
                          { icon: <Edit2 size={14} />,    act: () => setEditTarget(s), color: 'hover:bg-[#6a12b8] hover:text-white', tip: 'Specs' },
            ...
            <div className="flex items-center gap-3">
              <div className="flex -space-x-2">
                {[...Array(Math.min(3, filtered.length))].map((_, i) => (
                  <div key={i} className="w-6 h-6 rounded-full bg-[#6a12b8] border-2 border-white flex items-center justify-center text-[8px] font-black text-white">BR</div>
                ))}
              </div>
