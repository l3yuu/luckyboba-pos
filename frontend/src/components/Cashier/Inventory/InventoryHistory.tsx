...
const InventoryHistoryModal = ({ onClose }: InventoryHistoryModalProps) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* ── Header ── */}
        <div className="flex items-center justify-between px-7 py-5 border-b border-[#e9d5ff] bg-[#6a12b8] rounded-t-xl shrink-0">
          <div>
        ...
        <div className="px-7 py-3 shrink-0">
          <div className="relative">
            <input
              ...
              className="w-full bg-[#f5f0ff] border border-[#e9d5ff] rounded-lg px-10 py-2.5 text-xs font-semibold text-[#1c1c1e] outline-none focus:border-[#6a12b8] focus:bg-white transition-all placeholder:text-zinc-400"
            />
        ...
        <div className="flex-1 overflow-y-auto px-7 pb-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#6a12b8]" />
              <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-zinc-400">Loading history...</p>
            </div>
          ) : error ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              <p className="text-sm font-bold text-red-500">{error}</p>
              <button onClick={fetchMovements} className="h-9 px-5 bg-[#6a12b8] text-white font-bold text-xs uppercase tracking-widest rounded-lg">Retry</button>
            </div>
          ) : paginated.length === 0 ? (
                ...
                {paginated.map(m => {
                  ...
                  return (
                    <tr key={m.id} className="hover:bg-[#f5f0ff] transition-colors group">
                      ...
                      <td className="py-3 pr-4">
                        <span className="text-[12px] font-extrabold text-[#6a12b8]">
            ...
            {totalPages > 1 && (
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={page === 1}
                  className="h-8 w-8 flex items-center justify-center border border-[#e9d5ff] rounded-lg text-[#6a12b8] hover:border-[#6a12b8] hover:bg-[#f5f0ff] transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                >
