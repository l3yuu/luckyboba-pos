...
const COLORS = ['#a020f0', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6'];
...
const inputCls = (hasError?: boolean) =>
  `w-full px-4 py-3 rounded-[0.625rem] border text-sm font-semibold outline-none transition-all bg-white text-[#1c1c1e] placeholder:text-zinc-400 focus:border-[#a020f0] focus:bg-white ${hasError ? 'border-red-400' : 'border-[#e9d5ff]'}`;
const selectCls = `w-full px-4 py-3 rounded-[0.625rem] border border-[#e9d5ff] bg-white text-[#1c1c1e] font-semibold text-sm outline-none focus:border-[#a020f0] cursor-pointer`;
      ...
      <div className="bg-white rounded-[0.625rem] border border-[#e9d5ff] shadow-2xl w-full max-w-lg flex flex-col overflow-hidden" style={dashboardFont}>
        ...
        <div className="px-7 py-6 flex flex-col gap-5">
          ...
          <div className="space-y-1.5">
            ...
            <textarea value={form.notes} onChange={(e) => setForm(f => ({ ...f, notes: e.target.value }))} className="w-full px-4 py-3 rounded-[0.625rem] border border-[#e9d5ff] text-sm font-semibold outline-none transition-all bg-white text-[#1c1c1e] placeholder:text-zinc-400 focus:border-[#a020f0] h-20 resize-none" placeholder="Optional notes..." />
          </div>
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={form.is_intermediate} onChange={(e) => setForm(f => ({ ...f, is_intermediate: e.target.checked }))} className="w-4 h-4 accent-[#a020f0]" />
        ...
        <div className="flex gap-3 px-7 py-5 border-t border-[#e9d5ff]">
          ...
          <button onClick={handleSubmit} disabled={submitting} className="flex-1 h-11 bg-[#a020f0] text-white font-bold text-xs uppercase tracking-widest hover:bg-[#6a12b8] transition-all disabled:opacity-60 flex items-center justify-center gap-2 rounded-[0.625rem]">
      ...
      <div className="bg-white rounded-[0.625rem] border border-[#e9d5ff] shadow-2xl w-full max-w-md flex flex-col overflow-hidden" style={dashboardFont}>
        ...
        <div className="px-7 py-6 flex flex-col gap-5">
          <div className="bg-[#f5f0ff] px-4 py-3 border border-[#e9d5ff] rounded-[0.625rem]">
            <p className="text-[11px] font-bold text-[#a020f0] uppercase tracking-widest">{item.name}</p>
            ...
            <div className="grid grid-cols-4 gap-2">
              {(['add', 'subtract', 'waste', 'set'] as const).map((t) => (
                <button key={t} onClick={() => {
                  setType(t);
                  setReasonSelect('');
                }} className={`h-10 text-[10px] font-bold uppercase tracking-widest rounded-[0.625rem] border transition-all ${type === t ? 'bg-[#a020f0] text-white border-[#a020f0]' : 'bg-white text-zinc-500 border-[#e9d5ff] hover:border-[#a020f0]'}`}>
          ...
          {quantity && (
            <div className="flex items-center justify-between bg-[#f5f0ff] border border-[#e9d5ff] px-4 py-3 rounded-[0.625rem]">
              ...
              <span className={`text-sm font-extrabold ${previewStock < 0 ? 'text-red-500' : 'text-[#a020f0]'}`}>{previewStock.toFixed(4)} {item.unit}</span>
            </div>
          )}
          <div className="space-y-1.5">
          ...
          {reasonSelect === 'Other' && (
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full px-4 py-3 rounded-[0.625rem] border border-[#e9d5ff] text-sm font-semibold outline-none transition-all bg-white text-[#1c1c1e] placeholder:text-zinc-400 focus:border-[#a020f0] h-20 resize-none"
        ...
        <div className="flex gap-3 px-7 py-5 border-t border-[#e9d5ff]">
          ...
          <button onClick={handleSubmit} disabled={submitting || !quantity} className="flex-1 h-11 bg-[#a020f0] text-white font-bold text-xs uppercase tracking-widest hover:bg-[#6a12b8] transition-all disabled:opacity-60 flex items-center justify-center gap-2 rounded-[0.625rem]">
    ...
    <div ref={overlayRef} onClick={(e) => { if (e.target === overlayRef.current) onClose(); }} className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-[0.625rem] border border-[#e9d5ff] shadow-2xl w-full max-w-sm flex flex-col overflow-hidden" style={dashboardFont}>
        ...
        <div className="px-7 py-7 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-sm font-bold text-[#1c1c1e]">Delete <span className="text-[#a020f0]">"{item.name}"</span>?</p>
      ...
      <div className="bg-white w-full md:max-w-xl flex flex-col overflow-hidden border border-zinc-200 shadow-2xl" style={{ ...dashboardFont, maxHeight: '85vh' }}>
        ...
        <div className="grid grid-cols-4 border-b border-zinc-100 shrink-0">
          {[
            ...
            { label: 'Ending', value: fmt(row.ending), color: 'text-[#a020f0]' },
          ...
          <div className="flex items-center justify-between">
            ...
            {view === 'usage' && (
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => exportCSV(reportRows, periodLabel)} className="h-10 px-4 border border-[#a020f0] text-[#a020f0] font-bold text-xs uppercase tracking-widest hover:bg-[#f5f0ff] transition-all flex items-center gap-2">
          ...
          {view === 'dashboard' && (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  { label: 'Total Items', value: reportStats.totalItems, sub: 'tracked materials', color: 'text-[#a020f0]', bg: 'bg-white' },
                    ...
                    {salesData.length > 0 ? (
                      <ResponsiveContainer width="100%" height={220}>
                        <BarChart data={salesData}>
                          ...
                          <Bar dataKey="total_cost" name="Cost" fill="#a020f0" radius={[4, 4, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
              ...
              <div className="bg-white border border-zinc-200 overflow-hidden shadow-sm rounded-[0.625rem] mb-6">
                <div className="bg-white px-7 py-5 border-b border-zinc-100 flex flex-col md:flex-row items-center justify-between gap-4">
                  <h2 className="text-[#a020f0] font-black text-xs uppercase tracking-[0.15em]">TOP PRODUCTS ({start} - {end})</h2>
                  <div className="flex items-center gap-2">
                    ...
                    <div className="relative">
                      <input 
                        ...
                        className="border border-[#e9d5ff] bg-white px-4 py-2 text-xs outline-none focus:border-[#a020f0] w-48 md:w-64 font-semibold text-[#1c1c1e] rounded-[0.625rem] placeholder:text-zinc-300 shadow-sm transition-all"
                      />
                    </div>
                  </div>
                </div>
                <div className="overflow-x-auto min-h-[350px]">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-zinc-100">
                      {salesLoading ? (
                        <tr><td colSpan={8} className="py-20 text-center"><div className="flex flex-col items-center gap-2"><div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#a020f0]" /><span className="text-zinc-400 font-bold uppercase text-[10px]">Loading...</span></div></td></tr>
                        ...
                        <tr key={index} className="hover:bg-[#f5f0ff] transition-colors">
                          ...
                          <td className="px-7 py-3.5 text-center"><span className="text-[13px] font-extrabold text-[#a020f0]">{item.qty}</span></td>
                          ...
                          <td className="px-7 py-3.5 text-center"><span className="text-[13px] font-extrabold text-[#a020f0]">{formatPHP(item.sold_total)}</span></td>
                          ...
                          <td className="px-7 py-3.5"><span className="text-[13px] font-extrabold text-[#a020f0] uppercase tracking-tight">{item.name}</span></td>
                    ...
                    <tfoot className="bg-white border-t-2 border-zinc-100">
                      <tr className="font-black">
                        ...
                        <td className="px-7 py-4 text-center"><span className="text-[13px] font-extrabold text-[#a020f0]">{formatPHP(totals.sold)}</span></td>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 bg-white border border-zinc-200 px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                <div className="flex items-center gap-4">
                  ...
                  {[{ label: 'BEG', desc: 'Beginning stock (reconstructed)' }, { label: 'DEL', desc: 'Stock received/delivered this period' }, { label: 'COOKED', desc: 'Added as cooked or mixed' }, { label: 'OUT', desc: 'Manual deductions / used' }, { label: 'SPOIL', desc: 'Spoilage / waste' }, { label: 'END', desc: 'Current live stock' }, { label: 'USAGE', desc: 'Computed consumption' }, { label: 'VAR', desc: 'Variance (END − expected)' }]
                    .map(c => <span key={c.label} title={c.desc} className="cursor-help border-b border-dashed border-zinc-300 hover:text-[#a020f0] hover:border-[#a020f0] transition-colors">{c.label}</span>)}
                </div>
                ...
                <div className="flex items-center gap-2 px-2 py-0.5 bg-[#f5f0ff] border border-[#e9d5ff] rounded-full">
                  <span className="relative flex h-1.5 w-1.5">
                    ...
                    <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-[#a020f0]"></span>
                  </span>
                  <span className="text-[8px] font-black text-[#a020f0]">Live Sync</span>
                ...
                <div className="px-6 py-4 border-b border-[#e9d5ff] bg-[#f5f0ff] flex flex-col md:flex-row justify-between items-center gap-3">
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                      <span>Show</span>
                      <select value={usageEntriesLimit} onChange={e => setUsageEntriesLimit(Number(e.target.value))} className="border border-[#e9d5ff] bg-white px-2 py-1.5 outline-none text-[#1c1c1e] font-semibold text-xs focus:border-[#a020f0] rounded-[0.625rem]">
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                      <span>Category</span>
                      <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="border border-[#e9d5ff] bg-white px-2 py-1.5 outline-none text-[#1c1c1e] font-semibold text-xs focus:border-[#a020f0] rounded-[0.625rem]">
                        {usageCategories.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                    <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                      <span>Variance</span>
                      <select value={varianceFilter} onChange={e => setVarianceFilter(e.target.value as typeof varianceFilter)} className="border border-[#e9d5ff] bg-white px-2 py-1.5 outline-none text-[#1c1c1e] font-semibold text-xs focus:border-[#a020f0] rounded-[0.625rem]">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input type="text" value={usageSearch} onChange={e => setUsageSearch(e.target.value)} placeholder="Find item..." className="border border-[#e9d5ff] bg-white px-4 py-2 text-sm outline-none focus:border-[#a020f0] w-48 font-semibold text-[#1c1c1e] placeholder:text-zinc-400 rounded-[0.625rem]" />
                  </div>
                </div>

                <div className="overflow-auto">
                  {reportLoading ? (
                    <div className="py-16 flex flex-col items-center gap-2"><div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#a020f0]" /><span className="text-zinc-400 font-bold uppercase text-[10px]">Loading report...</span></div>
                  ) : (
                    <table className="w-full text-left border-collapse min-w-225">
                      <thead className="sticky top-0 bg-white z-10 border-b-2 border-[#e9d5ff]">
                        <tr className="bg-[#f5f0ff]">
                          ...
                          <th className="px-4 py-3.5 text-[9px] font-bold text-[#a020f0] uppercase tracking-widest text-right w-20 bg-[#f5f0ff] border-r border-[#e9d5ff]">END</th>
                            ...
                            <tr key={row.material.id} className={`transition-colors ${isLow ? 'bg-red-50/40 hover:bg-red-50' : 'hover:bg-[#f5f0ff]'}`}>
                              ...
                              <td className="px-5 py-3">
                                <div className="flex items-center gap-2 flex-wrap">
                                  <span className="text-[12px] font-extrabold text-[#a020f0]">{row.material.name}</span>
                              ...
                              <td className="px-4 py-3 text-right bg-zinc-50/70 border-r border-zinc-100"><span className={`text-[13px] font-extrabold ${isLow ? 'text-red-600' : 'text-[#a020f0]'}`}>{fmt(row.ending)}</span></td>
                              ...
                              <td className="px-5 py-3 text-center">
                                <button onClick={() => setDrawerRow(row)} title="View stock movements" className="h-8 w-8 inline-flex items-center justify-center border border-[#e9d5ff] text-zinc-400 hover:border-[#a020f0] hover:text-[#a020f0] hover:bg-[#f5f0ff] transition-all">
            ...
            <div className="flex-1 bg-white border border-zinc-200 overflow-hidden flex flex-col shadow-sm rounded-[0.625rem]">
              <div className="px-6 py-4 border-b border-[#e9d5ff] bg-[#f5f0ff] flex flex-col md:flex-row justify-between items-center gap-3">
                <div className="flex items-center gap-3 flex-wrap">
                  <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                    <span>Show</span>
                    <select value={matEntriesLimit} onChange={e => setMatEntriesLimit(Number(e.target.value))} className="border border-[#e9d5ff] bg-white px-2 py-1.5 outline-none text-[#1c1c1e] font-semibold text-xs rounded-[0.625rem] focus:border-[#a020f0]">
                  ...
                  <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                    <span>Category</span>
                    <select value={matCategory} onChange={e => setMatCategory(e.target.value)} className="border border-[#e9d5ff] bg-white px-2 py-1.5 outline-none text-[#1c1c1e] font-semibold text-xs rounded-[0.625rem] focus:border-[#a020f0]">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Search:</span>
                  <input type="text" value={matSearch} onChange={e => setMatSearch(e.target.value)} placeholder="Find item..." className="border border-[#e9d5ff] bg-white px-4 py-2 text-sm outline-none focus:border-[#a020f0] w-56 font-semibold text-[#1c1c1e] rounded-[0.625rem] placeholder:text-zinc-400" />
                </div>
              </div>
              <div className="flex-1 overflow-auto">
                {materialsLoading && materials.length === 0 ? (
                  <div className="py-16 flex flex-col items-center gap-2"><div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#a020f0]" /><span className="text-zinc-400 font-bold uppercase text-[10px]">Loading...</span></div>
                ) : (
                  <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 bg-white z-10 border-b-2 border-[#e9d5ff]">
                      <tr className="bg-[#f5f0ff]">
          ...
          {view === 'recipes' && (
            <>
              <div className="grid grid-cols-3 gap-3">
                {[
                  { label: 'Total Rows', value: recipeStats.total, color: 'text-[#a020f0]' },
              ...
              <div className="flex-1 bg-white border border-zinc-200 overflow-hidden flex flex-col shadow-sm rounded-[0.625rem]">
                <div className="px-6 py-4 border-b border-[#e9d5ff] bg-[#f5f0ff] flex flex-col md:flex-row justify-between items-center gap-3">
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                      <span>Show</span>
                      <select value={recipeEntriesLimit} onChange={e => setRecipeEntriesLimit(Number(e.target.value))} className="border border-[#e9d5ff] bg-white px-2 py-1.5 outline-none text-[#1c1c1e] font-semibold text-xs rounded-[0.625rem] focus:border-[#a020f0]">
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                      <span>Filter</span>
                      <select value={recipeFilterStatus} onChange={e => setRecipeFilterStatus(e.target.value as 'all' | 'with' | 'without')} className="border border-[#e9d5ff] bg-white px-2 py-1.5 outline-none text-[#1c1c1e] font-semibold text-xs rounded-[0.625rem] focus:border-[#a020f0]">
                        <option value="all">All Items</option><option value="with">Has Recipe</option><option value="without">Missing Recipe</option>
                      </select>
                    </div>
                    <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                      <span>Category</span>
                      <select value={recipeCategoryFilter} onChange={e => setRecipeCategoryFilter(e.target.value)} className="border border-[#e9d5ff] bg-white px-2 py-1.5 outline-none text-[#1c1c1e] font-semibold text-xs rounded-[0.625rem] focus:border-[#a020f0]">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Search:</span>
                    <input type="text" value={recipeSearch} onChange={e => setRecipeSearch(e.target.value)} placeholder="Find menu item..." className="border border-[#e9d5ff] bg-white px-4 py-2 text-sm outline-none focus:border-[#a020f0] w-56 font-semibold text-[#1c1c1e] rounded-[0.625rem] placeholder:text-zinc-400" />
                  </div>
                </div>
                      ...
                      <tbody className="divide-y divide-zinc-100">
                        {displayRecipeRows.length > 0 ? displayRecipeRows.map((row, idx) => (
                          <tr key={`${row.menuItem.id}-${row.sizeLabel}-${idx}`} className={`transition-colors ${!row.hasRecipe ? 'bg-amber-50/40 hover:bg-amber-50' : 'hover:bg-[#f5f0ff]'}`}>
                            <td className="px-7 py-3.5">
                              <span className="text-[13px] font-extrabold text-[#a020f0]">{row.menuItem.name}</span>
                              ...
                              {row.recipe ? (
                                <div className="flex flex-wrap gap-1">
                                  {row.recipe.items.map((ri: RecipeItem) => (
                                    <span key={ri.id} className="text-[10px] font-semibold bg-[#f5f0ff] text-[#a020f0] px-2 py-0.5 border border-[#e9d5ff]">
