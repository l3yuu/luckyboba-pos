      ...
      <div className="px-6 pt-6 flex gap-4 border-b border-[#e9d5ff] bg-white">
        <button 
          ...
          className={`pb-3 text-[10px] font-black uppercase tracking-widest transition-all ${
            activeTab === 'upload' ? 'border-b-2 border-[#a020f0] text-[#a020f0]' : 'text-zinc-400 hover:text-zinc-600'
          }`}
        >
          Upload New
        </button>
        <button 
          ...
          className={`pb-3 text-[10px] font-black uppercase tracking-widest transition-all ${
            activeTab === 'history' ? 'border-b-2 border-[#a020f0] text-[#a020f0]' : 'text-zinc-400 hover:text-zinc-600'
          }`}
        >
          Import History
        </button>
        ...
        {activeTab === 'upload' ? (
          <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
            <div className="flex justify-between items-center">
              <h1 className="text-xl font-black text-[#a020f0] uppercase tracking-widest">Bulk Import</h1>
              <button 
                onClick={downloadTemplate} 
                className="h-11 bg-[#a020f0] hover:bg-[#6a12b8] text-white font-bold text-xs uppercase tracking-widest transition-colors rounded-[0.625rem] shadow-sm flex items-center justify-center gap-2 px-6"
              >
            ...
            <div 
              ...
              className={`bg-white p-12 rounded-[0.625rem] border-2 border-dashed transition-all duration-200 ${
                selectedFile 
                  ...
                  : isDragging 
                    ? 'border-[#a020f0] bg-[#f5f0ff] scale-[1.01] shadow-md' 
                    : 'border-[#e9d5ff] hover:border-[#a020f0] cursor-pointer'
            ...
            >
              ...
              {selectedFile ? (
                ...
                <>
                  <div className={`p-4 rounded-full transition-colors ${isDragging ? 'bg-[#f5f0ff] text-[#a020f0]' : 'bg-zinc-50 text-zinc-300'}`}>
                    <Upload size={32} />
                  </div>
                  <div className="text-center">
                    ...
                    <tbody className="divide-y divide-zinc-50">
                      {preview.map((row, idx) => (
                        <tr key={idx} className="hover:bg-[#f5f0ff]">
                          <td className="px-4 py-2 text-[10px] font-bold text-[#a020f0] uppercase tracking-tighter">{row.name}</td>
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            <h1 className="text-xl font-black text-[#a020f0] uppercase tracking-widest flex items-center gap-2">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-zinc-100">
                    {isLoadingHistory ? (
                      <tr><td colSpan={3} className="p-12 text-center"><Loader2 className="animate-spin mx-auto text-[#a020f0]" /></td></tr>
                      ...
                      <tr key={log.id} className="hover:bg-[#f5f0ff] transition-colors">
                        ...
                        <td className="px-6 py-4 text-[11px] font-black text-[#a020f0] uppercase tracking-tighter">
                          {log.action}
                        </td>
        ...
        <div className="flex gap-3 pt-4 border-t border-[#e9d5ff]">
          {activeTab === 'upload' && (
            <button 
              ...
              className="h-11 bg-[#a020f0] hover:bg-[#6a12b8] text-white font-bold text-xs uppercase tracking-widest transition-colors rounded-[0.625rem] shadow-sm flex items-center justify-center gap-2 px-6 disabled:opacity-50"
            >
          ...
          <button
            onClick={onBack}
            className="h-11 bg-white border border-[#e9d5ff] text-[#a020f0] hover:bg-[#f5f0ff] font-bold text-xs uppercase tracking-widest transition-colors rounded-[0.625rem] shadow-sm flex items-center justify-center gap-2 px-6"
          >
      ...
      {summary && (
        <div className="fixed inset-0 z-150 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-[0.625rem] shadow-2xl overflow-hidden">
            <div className="bg-[#a020f0] p-4 flex justify-between items-center text-white rounded-t-[0.625rem]">
            ...
            <div className="p-8 space-y-6">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <button 
                      ...
                      className="text-[9px] font-black text-[#a020f0] uppercase hover:underline flex items-center gap-1"
                    >
              ...
              <button 
                ...
                className="w-full h-11 bg-[#a020f0] hover:bg-[#6a12b8] text-white font-bold text-xs uppercase tracking-widest transition-colors rounded-[0.625rem] shadow-sm flex items-center justify-center gap-2"
              >
                Close Report
              </button>
            </div>
