            ...
            <div className="flex flex-col xl:flex-row xl:items-end gap-4">
              ...
              <div className="flex-1">
                ...
                <select 
                  ...
                  onChange={(e) => setSelectedMainCategoryId(Number(e.target.value))}
              ...
              <button 
                ...
                disabled={loading}
                className="bg-[#1e40af] hover:bg-[#1e3a8a] text-white px-8 h-10 rounded-md font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-md disabled:opacity-50"
              >
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-zinc-100">
                {subCategories.length > 0 ? (
                  subCategories.map((sub, index) => (
                    <tr key={sub.id} className={`hover:bg-blue-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-zinc-50/30'}`}>
                      ...
                      <td className="px-6 py-4 text-center">
                        <button className="bg-[#1e40af] hover:bg-blue-700 text-white p-2 rounded-lg active:scale-95"><Edit3 size={14} /></button>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <button onClick={() => handleDelete(sub.id)} className="bg-red-600 hover:bg-red-700 text-white p-2 rounded-lg active:scale-95"><Trash2 size={14} /></button>
                      </td>
                    </tr>
                  ))
                ) : (
                  !isFetching && (
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-zinc-400 font-bold text-xs uppercase">No sub-categories found</td>
                    </tr>
                  )
                )}
              </tbody>
