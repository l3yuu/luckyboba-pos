...
const SalesTile = ({ label, value, icon: Icon, color, trend }: SalesTileProps) => (
  <div className="sv-tile sv-card p-6 flex flex-col justify-between min-h-[150px]">
    <div className="flex items-start justify-between">
      <div 
        className="w-11 h-11 rounded-2xl flex items-center justify-center shadow-lg shadow-[#a020f0]/10 border" 
  ...
  return (
    <div className="px-5 md:px-8 py-8 md:py-12 sv-analytics animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12 pb-10 border-b border-slate-100">
        <div>
           <div className="flex items-center gap-3 mb-3">
             ...
             <p className="sv-label !text-[#a020f0] font-black tracking-widest">Global Sales Audit Hub</p>
           </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            onClick={() => load(true)} 
            className="p-3.5 bg-white border border-slate-200 text-slate-500 hover:text-[#a020f0] hover:border-[#a020f0]/30 rounded-2xl transition-all shadow-xl shadow-[#a020f0]/5 group"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
        <SalesTile label="Aggregate Revenue" value={fmtS(totalSales)} icon={PhilippinePeso} color="#a020f0" trend={+14.2} />
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
               <LineIcon size={16} className="text-[#a020f0]" />
          ...
          <div className="flex items-center gap-6 px-6 py-3 bg-[#a020f0]/10/50 border border-[#a020f0]/20/50 rounded-2xl backdrop-blur-sm">
            <div className="flex items-center gap-2.5">
              <span className="w-2.5 h-2.5 rounded-full bg-[#a020f0] shadow-lg shadow-[#a020f0]/40" />
              <span className="text-[10px] font-black text-[#a020f0] uppercase tracking-widest">Gross Yield</span>
            </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
              ...
              <Area 
                ...
                dot={{ r: 6, strokeWidth: 3, fill: '#fff', stroke: '#a020f0' }}
                activeDot={{ r: 9, strokeWidth: 4, fill: '#a020f0', stroke: '#fff' }}
      ...
      <div className="sv-card overflow-hidden border border-slate-100 bg-white">
        <div className="px-10 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[#a020f0] flex items-center justify-center text-white shadow-lg shadow-[#a020f0]/20"><FileText size={18} strokeWidth={2.5}/></div>
          ...
          <div className="flex items-center gap-2 px-3 py-1 bg-white border border-slate-200 rounded-lg text-[9px] font-black text-slate-400 uppercase tracking-widest">
            <RefreshCw size={10} className="text-[#a020f0]" /> Auto-Reconciled
          </div>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {data.slice().reverse().map((row, idx) => (
                <tr key={idx} className="hover:bg-[#a020f0]/10/30 transition-colors duration-300">
                  ...
                  <td className="px-10 py-6 font-black text-[#a020f0] tabular-nums text-sm">{fmt(row.sales)}</td>
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 h-2 bg-slate-100 rounded-full w-28 hidden sm:block overflow-hidden shadow-inner">
                        <div 
                          className="h-full bg-gradient-to-r from-[#a020f0] to-[#a020f0] rounded-full transition-all duration-1000" 
