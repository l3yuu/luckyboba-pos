...
const STYLES = `
  ...
  .sv-header-strip {
    background: linear-gradient(135deg, #0f172a 0%, #1e1b4b 50%, #a020f0 100%);
...
const SV_GRADIENT = ['#a020f0', '#4a2c7a', '#5a3891', '#6a44a7', '#7a50be', '#8a5cd5'];
...
const ItemsReportPanel = ({ branchId }: { branchId: number | null }) => {
  ...
  if (loading) return (
    <div className="h-full min-h-[60vh] flex flex-col items-center justify-center gap-4">
      <style>{STYLES}</style>
      <div className="w-12 h-12 border-4 border-[#a020f0] border-t-transparent rounded-full sv-spin" />
      <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#a020f0]/50">Establishing Secure Uplink</p>
    </div>
  );
  ...
  const stats = [
    { label: 'Inventory Throughput', value: totalQuantity.toLocaleString(), icon: <ShoppingBag size={14} />, color: '#a020f0' },
    { label: 'Gross Item Yield', value: `₱${totalRevenue.toLocaleString()}`, icon: <TrendingUp size={14} />, color: '#a020f0' },
    { label: 'Unit Economics', value: `₱${(totalRevenue / (totalQuantity || 1)).toFixed(2)}`, icon: <Target size={14} />, color: '#a020f0' },
    { label: 'Product Diversity', value: new Set(data.map(d => d.category)).size, icon: <Award size={14} />, color: '#a020f0' },
  ];
  ...
  return (
    <div className="sv-items-report px-5 md:px-8 pb-20">
      ...
      <div className="sv-header-strip">
        ...
        <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-[#a020f0]/[0.05] rounded-full -ml-32 -mb-32 blur-2xl pointer-events-none" />
      ...
      <div className="px-8 md:px-12 -mt-20 relative z-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          {stats.map((s, i) => (
            <div key={i} className="sv-glass-card p-6 flex items-center gap-5 border-none shadow-2xl shadow-[#a020f0]/10 transition-transform hover:scale-[1.03]">
        ...
        <div className="sv-glass-card p-6 mb-10 border-none flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl shadow-slate-900/5">
          <div>
            <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
              <Filter size={14} className="text-[#a020f0]" /> Metric Ranking Logic
            </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-10">
          ...
          <div className="lg:col-span-2 sv-glass-card p-8 border-none shadow-2xl shadow-[#a020f0]/10">
            <div className="flex items-center justify-between mb-12">
               <div>
                 <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.15em] flex items-center gap-2">
                    <BarChart3 size={16} className="text-[#a020f0]" /> Top Performer Distribution
                 </h3>
                 <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">Comparative saturation audit</p>
               </div>
               <span className="px-3 py-1 bg-[#a020f0]/10 text-[#a020f0] rounded-lg text-[10px] font-black uppercase tracking-[0.2em]">{data.length} Nodes</span>
            </div>
          ...
          <div className="sv-glass-card p-8 border-none shadow-2xl shadow-[#a020f0]/10">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.15em] mb-8 flex items-center gap-2">
               <Award size={18} className="text-[#a020f0]" /> Leaderboard
            </h3>
               ...
               {sortedData.slice(0, 6).map((item, i) => (
                 <div key={i} className="group">
                    <div className="flex items-center justify-between mb-2">
                       <div className="flex items-center gap-4">
                          <span className={`w-8 h-8 rounded-xl flex items-center justify-center text-[11px] font-black shadow-md ${i === 0 ? 'bg-[#a020f0] text-white' : 'bg-white border border-slate-100 text-slate-400 group-hover:text-[#a020f0]'}`}>
                            {i + 1}
                          </span>
                          <span className="text-[12px] font-black text-slate-800 tracking-tight">{item.name}</span>
                       </div>
                       <span className="text-[10px] font-black text-[#a020f0] tabular-nums">
                         {sortBy === 'quantity' ? `${item.quantity} SOLD` : `₱${item.revenue.toLocaleString()}`}
                       </span>
                    </div>
                    <div className="sv-progress-bg group-hover:bg-[#a020f0]/10 transition-colors shadow-inner">
          ...
          <div className="px-10 py-7 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div>
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em] flex items-center gap-2">
                <FileText size={16} className="text-[#a020f0]" /> Integrated performance matrix
              </h3>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {sortedData.map((item, index) => (
                  <tr key={index} className="group hover:bg-[#a020f0]/10/20 transition-all duration-300 cursor-default">
                    <td className="px-10 py-6">
                      <div className="flex items-center gap-4">
                        <span className={`w-8 h-8 rounded-xl flex items-center justify-center text-[10px] font-black transition-all ${index < 3 ? 'bg-[#a020f0] text-white shadow-lg shadow-[#a020f0]/20' : 'bg-slate-50 border border-slate-100 text-slate-400 group-hover:bg-white group-hover:text-[#a020f0]'}`}>{index + 1}</span>
                    ...
                    <td className="px-10 py-6">
                       <span className="px-3 py-1.5 bg-slate-100 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-widest group-hover:bg-[#a020f0]/10 group-hover:text-[#a020f0] transition-colors">{item.category}</span>
                    </td>
                    <td className="px-10 py-6 text-right font-black text-slate-700 tabular-nums text-sm">{item.quantity}</td>
                    <td className="px-10 py-6 text-right font-black text-[#a020f0] tabular-nums text-sm">₱{item.revenue.toLocaleString()}</td>
                    <td className="px-10 py-6 text-right">
                       <div className="flex items-center justify-end gap-2">
                          ...
                          <ArrowRight size={12} className="text-slate-200 group-hover:translate-x-2 group-hover:text-[#a020f0] transition-all" />
                       </div>
                    </td>
                  </tr>
                ))}
