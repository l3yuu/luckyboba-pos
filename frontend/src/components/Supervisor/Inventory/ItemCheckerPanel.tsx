...
const ItemCheckerPanel = ({ branchId }: { branchId: number | null }) => {
  ...
  if (loading) return (
    <div className="h-full min-h-[60vh] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#a020f0]/50">Synchronizing Menu Matrix</p>
    </div>
  );
  ...
  return (
    <div className="sv-checker px-5 md:px-8 py-8 md:py-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12 pb-10 border-b border-slate-100">
        <div>
          <div className="flex items-center gap-3 mb-3">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#a020f0]">Product Integrity Watch</p>
          </div>
          ...
          <p className="text-[0.8rem] font-bold text-slate-400 mt-4 flex items-center gap-2">
            <Tag size={14} className="text-[#a020f0]" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
        {[
          { label: 'Total Catalog', count: items.length, color: '#a020f0', icon: Package },
      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-8">
        <div className="flex-[2] sv-glass border-none shadow-xl shadow-slate-900/5 px-6 py-4 flex items-center gap-4 bg-white focus-within:ring-8 focus-within:ring-[#a020f0]/5 transition-all">
        ...
        <div className="flex-1">
          <select
            ...
            className="w-full sv-glass border-none shadow-xl shadow-slate-900/5 px-6 py-4 bg-white font-black text-[11px] uppercase tracking-widest text-slate-600 outline-none focus:ring-4 focus:ring-[#a020f0]/10 transition-all appearance-none cursor-pointer"
      ...
      {/* ── DATA GRID ── */}
      <div className="sv-glass border-none overflow-hidden shadow-2xl shadow-[#a020f0]/5 bg-white">
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em] flex items-center gap-2">
              <ShoppingBag size={16} className="text-[#a020f0]" /> Catalog Registry
            </h3>
          ...
          <div className="flex items-center gap-2 px-3 py-1 bg-[#a020f0] text-white border border-slate-200 rounded-lg text-[9px] font-black uppercase tracking-widest">
              ...
              {filteredItems.map((item) => (
                <tr key={item.id} className="group hover:bg-[#a020f0]/10/20 transition-all duration-300">
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-slate-50 group-hover:bg-white flex items-center justify-center text-slate-400 group-hover:text-[#a020f0] transition-all shadow-sm">
                        <ShoppingBag size={18} />
                      </div>
                  ...
                  <td className="px-10 py-6">
                    <span className="px-3 py-1.5 bg-slate-100 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-widest group-hover:bg-[#a020f0] group-hover:text-white transition-all">{item.category}</span>
                  </td>
