...
const StockLevelsPanel = ({ branchId }: { branchId: number | null }) => {
  ...
  if (loading) return (
    <div className="h-full min-h-[60vh] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#6a12b8]/50">Querying Logistics Nodes</p>
    </div>
  );
  ...
  return (
    <div className="sv-inventory px-5 md:px-8 py-8 md:py-12 animate-in fade-in duration-700">
      ...
      {/* ── COMMAND HEADER ── */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12 pb-10 border-b border-[#6a12b8]/20/50">
        <div>
           <div className="flex items-center gap-3 mb-3">
             <div className="w-2.5 h-2.5 rounded-full bg-[#6a12b8]" />
             <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#6a12b8]">Stock Integrity Hub</p>
           </div>
           ...
           <p className="text-[0.8rem] font-bold text-slate-400 mt-4 flex items-center gap-2">
            <Layers size={14} className="text-[#6a12b8]" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
        {[
          { label: 'Network Entities', count: items.length, color: '#6a12b8', icon: LayoutGrid },
      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-8">
        <div className="flex-1 sv-card border-none shadow-xl shadow-slate-900/5 px-6 py-4 flex items-center gap-4 bg-white focus-within:ring-8 focus-within:ring-[#6a12b8]/5 transition-all">
        ...
        <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 shadow-inner">
          {(['all', 'low', 'critical'] as const).map((s) => (
            <button
              key={s}
              onClick={() => setFilter(s)}
              style={filter === s ? { backgroundColor: '#6a12b8' } : {}}
      ...
      {/* ── DATA GRID ── */}
      <div className="sv-card border-none overflow-hidden shadow-2xl shadow-[#6a12b8]/5 bg-white">
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em] flex items-center gap-2">
              <Package size={16} className="text-[#6a12b8]" /> Logistics Ledger
            </h3>
          ...
          <div className="flex items-center gap-2 px-3 py-1 bg-white border border-slate-200 rounded-lg text-[9px] font-black text-slate-400 uppercase tracking-widest">
            <RefreshCw size={10} className="text-[#6a12b8]" /> Real-time Link
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredItems.map((item) => (
                <tr key={item.id} className="group hover:bg-[#6a12b8]/10/20 transition-all duration-300">
                  ...
                  <td className="px-10 py-6">
                    <span className="px-3 py-1.5 bg-slate-100 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-widest group-hover:bg-white group-hover:text-[#6a12b8] transition-colors">{item.category}</span>
                  </td>
