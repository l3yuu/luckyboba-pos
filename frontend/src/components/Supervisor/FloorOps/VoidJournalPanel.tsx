...
const StatusBadge: React.FC<{ status: VoidStatus }> = ({ status }) => {
  const map: Record<string, { bg: string; text: string; icon: React.ReactNode }> = {
    ...
    completed: { bg: 'bg-[#6a12b8]/10 text-[#6a12b8] border-[#6a12b8]/20', icon: <FileText size={10} />, text: 'Legitimate' },
...
const VoidJournalPanel: React.FC<{ branchId: number | null }> = ({ branchId }) => {
  ...
  return (
    <div className="px-5 md:px-8 py-8 md:py-12 animate-in fade-in duration-700" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12 pb-10 border-b border-slate-100">
        ...
        <div className="flex items-center gap-4">
          <div className="relative group">
            <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6a12b8] group-hover:scale-110 transition-transform" size={16} />
            <input
              type="date"
              value={selectedDate}
              onChange={e => setSelectedDate(e.target.value)}
              className="bg-white border-2 border-slate-100 rounded-2xl px-12 py-3.5 text-xs font-black uppercase tracking-widest outline-none focus:border-[#6a12b8] transition-all shadow-xl shadow-slate-900/5"
            />
          </div>
          <button
            onClick={fetchLogs}
            ...
            className="inline-flex items-center gap-1.5 font-bold rounded-lg transition-all px-4 py-2 text-xs text-white hover:opacity-90 shadow-xl shadow-[#6a12b8]/20 disabled:opacity-50"
          >
      ...
      <div className="bg-white border-2 border-slate-100 rounded-[2.5rem] overflow-hidden shadow-2xl shadow-[#6a12b8]/5">
        <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/30">
          <div className="flex items-center gap-4 bg-white border-2 border-slate-100 rounded-[1.25rem] px-5 py-4 focus-within:border-[#6a12b8] focus-within:ring-8 focus-within:ring-indigo-50 transition-all">
                ...
                <tr key={log.id} className="group hover:bg-slate-50/80 transition-colors duration-300">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-3">
                      <div className="w-1.5 h-4 bg-slate-200 rounded-full group-hover:bg-[#6a12b8] transition-colors" />
