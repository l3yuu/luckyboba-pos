                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <button 
                      key={label} 
                      onClick={action}
                      className={`${group.color} text-white p-5 rounded-xl font-black text-[11px] uppercase tracking-widest shadow-sm hover:brightness-110 active:scale-[0.98] transition-all text-center min-h-17.5 flex items-center justify-center`}
                    >
                      {label}
                    </button>
                  );
