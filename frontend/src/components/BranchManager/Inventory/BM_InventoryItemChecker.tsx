      ...
      <div className="px-6 py-5 border-b border-zinc-100 bg-[#faf9ff] flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#f5f0ff] border border-[#e9d5ff] rounded-xl flex items-center justify-center">
            <Tag size={18} className="text-[#a020f0]" />
          </div>
          <div>
        ...
        <div className="text-right">
          <p className="text-xl font-black text-[#a020f0]">{formatPrice(item.price)}</p>
              ...
              return (
                <div key={bs.branch_id} className="flex items-center gap-3 p-3 bg-zinc-50 border border-zinc-100 rounded-xl">
                  <div className="w-7 h-7 bg-[#f5f0ff] border border-[#e9d5ff] rounded-lg flex items-center justify-center shrink-0">
                    <Building2 size={12} className="text-[#a020f0]" />
                  </div>
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-[0.625rem] overflow-hidden shadow-sm">
      <div className="px-6 py-5 border-b border-zinc-100 bg-[#faf9ff] flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#f5f0ff] border border-[#e9d5ff] rounded-xl flex items-center justify-center">
            <Package size={18} className="text-[#a020f0]" />
          </div>
          <div>
            <p className="text-base font-black text-[#1a0f2e]">{item.name}</p>
...
const BM_InventoryItemChecker: React.FC = () => {
  ...
  return (
    <div className="p-6 md:p-8 bg-[#f4f2fb] min-h-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-6 mb-8">
        <div className="flex-1 flex flex-col md:flex-row items-center gap-3">
          <div className="flex items-center gap-1.5 p-1 bg-white border border-zinc-200 rounded-xl overflow-hidden shrink-0">
            {(['menu', 'stock'] as SearchMode[]).map(m => (
              <button
                ...
                className={`px-4 py-2 rounded-lg font-bold text-[10px] uppercase tracking-widest transition-all ${mode === m ? 'bg-[#a020f0] text-white shadow-sm' : 'text-zinc-500 hover:text-[#a020f0] hover:bg-[#faf9ff]'}`}
              >
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 px-4 py-3 bg-white border rounded-xl font-bold text-[10px] uppercase tracking-widest transition-all shrink-0 w-full md:w-auto shadow-sm ${barcodeMode ? 'border-[#a020f0] text-[#a020f0] bg-[#f5f0ff]' : 'border-zinc-200 text-zinc-500 hover:border-[#a020f0] hover:text-[#a020f0]'}`}
          >
      ...
      <div className="flex items-center gap-3 mb-8">
        <div className={`flex items-center gap-3 bg-white border-2 rounded-[1rem] flex-1 shadow-sm transition-all ${barcodeMode ? 'border-[#a020f0]' : 'border-zinc-200 focus-within:border-[#a020f0]'}`}>
          <div className="pl-4 text-zinc-400 shrink-0">
            {barcodeMode ? <Barcode size={22} className="text-[#a020f0]" /> : <Search size={22} />}
          </div>
          <input
            ref={inputRef}
            type="text"
        ...
        <button
          ...
          className="bg-[#a020f0] hover:bg-[#2a1647] text-white px-8 h-16 rounded-[1rem] font-bold text-sm uppercase tracking-widest transition-all active:scale-[0.98] disabled:opacity-50 flex items-center gap-2 shadow-sm">
      ...
      {barcodeMode && (
        <div className="flex items-center gap-3 p-4 bg-[#f5f0ff] border border-[#e9d5ff] rounded-xl mb-5">
          <Barcode size={20} className="text-[#a020f0] shrink-0" />
          <div>
      ...
      {!loading && !hasSearched && (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          <div className="w-16 h-16 bg-[#f5f0ff] border border-[#e9d5ff] rounded-2xl flex items-center justify-center">
            <Search size={28} className="text-[#a020f0]" />
          </div>
          ...
          <div className="flex items-center gap-6 mt-2">
            {[
              ...
              { icon: <Tag         size={14} className="text-[#a020f0]"   />, label: 'Pricing info'   },
