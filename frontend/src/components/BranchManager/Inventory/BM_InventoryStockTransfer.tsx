    ...
    <div className="fixed inset-0 z-9999 flex items-center justify-center p-6"
      ...
      <div className="relative bg-white w-full max-w-2xl border border-zinc-200 rounded-[1.25rem] shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-100 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-[#f5f0ff] border border-[#e9d5ff] rounded-lg flex items-center justify-center">
              <ArrowRightLeft size={15} className="text-[#a020f0]" />
            </div>
            <div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1">
              <Field label="From Branch" required error={errors.from}>
                {branchId ? (
                  <div className="w-full text-sm text-[#a020f0] bg-[#f5f0ff] border border-[#e9d5ff] rounded-lg px-3 py-2.5 flex items-center gap-2 shadow-sm">
            ...
            <div className="pt-5 shrink-0">
              <div className="w-9 h-9 bg-[#f5f0ff] border border-[#e9d5ff] rounded-full flex items-center justify-center">
                <ArrowRightLeft size={14} className="text-[#a020f0]" />
              </div>
            </div>
            <div className="flex-1">
          ...
          {(fromBranch || toBranch) && (
            <div className="flex items-center gap-3 p-3 bg-zinc-50 border border-zinc-200 rounded-xl">
              ...
              <ArrowRightLeft size={16} className="text-[#a020f0] shrink-0" />
              <div className="flex-1 text-center">
        ...
        <div className="flex items-center gap-2 px-6 py-4 border-t border-zinc-100 shrink-0">
          ...
          <button onClick={handleSubmit} disabled={saving} className="flex-1 py-2.5 bg-[#a020f0] hover:bg-[#6a12b8] text-white rounded-lg font-bold text-xs uppercase tracking-widest transition-all disabled:opacity-50">{saving ? 'Creating...' : 'Create Transfer'}</button>
        </div>
      </div>
    </div>,
    document.body
      ...
      <div className="relative bg-white w-full max-w-md border border-zinc-200 rounded-[1.25rem] shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3 p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
            ...
            <ArrowRightLeft size={18} className="text-[#a020f0] shrink-0" />
            <div className="flex-1 text-center">
          ...
          <div className="border border-zinc-200 rounded-xl overflow-hidden">
            ...
            {items.map((item, i) => (
              <div key={i} className="grid grid-cols-[1fr_80px_60px] px-4 py-2.5 border-b border-zinc-100 last:border-0">
                ...
                <p className="text-xs font-bold text-[#a020f0] tabular-nums">{item.quantity}</p>
    ...
    <div className="p-6 md:p-8 bg-[#f4f2fb] min-h-full">
      ...
      <div className="grid grid-cols-5 gap-3 mb-5">
        {(Object.entries(counts) as [TransferStatus, number][]).map(([status, count]) => {
          const c = STATUS_CONFIG[status];
          return (
            <div key={status} className="bg-white border rounded-[0.625rem] px-4 py-3.5 shadow-sm cursor-pointer hover:border-[#a020f0] transition-colors" style={{ borderColor: statusFilter === status ? '#a020f0' : c.border }}
                ...
                return (
                  <React.Fragment key={t.id}>
                    <tr className={`border-b border-zinc-50 hover:bg-[#faf9ff] transition-colors ${isIncoming && (t.status === 'Approved' || t.status === 'In Transit') ? 'bg-emerald-50/30' : ''}`}>
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          <span className={`text-[11px] font-bold ${isOutgoing ? 'text-[#a020f0]' : 'text-zinc-500'}`}>
                            {t.from_branch?.name ?? t.from_branch_name ?? '—'}
                          </span>
                          <ArrowRight size={10} className="text-zinc-300" />
                          <span className={`text-[11px] font-bold ${isIncoming ? 'text-[#a020f0] bg-[#f5f0ff] px-2 py-0.5 rounded border border-[#e9d5ff]' : 'text-zinc-500'}`}>
                      ...
                      <td className="px-5 py-3.5">
                        <span className="text-[10px] font-bold text-[#a020f0] bg-[#f5f0ff] px-2 py-0.5 rounded border border-[#e9d5ff] uppercase tracking-tighter">{items.length} items</span>
                      </td>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => setExpanded(isExp ? null : t.id)} className="p-1.5 hover:bg-[#f5f0ff] rounded-[0.4rem] text-zinc-400 hover:text-[#a020f0] transition-colors">
                            <ChevronDown size={13} className={`transition-transform ${isExp ? 'rotate-180' : ''}`} />
                          </button>
                          <button onClick={() => setViewTarget(t)} className="p-1.5 hover:bg-[#f5f0ff] rounded-[0.4rem] text-zinc-400 hover:text-[#a020f0] transition-colors">
                            <Eye size={13} />
                          </button>
                        </div>
                      </td>
                        ...
                        <td colSpan={7} className="px-5 pb-3 pt-1">
                          <div className="ml-10 border border-[#e9d5ff] rounded-xl overflow-hidden">
                            <div className="grid grid-cols-3 bg-[#f5f0ff] px-4 py-2 border-b border-[#e9d5ff]">
                              {['Material', 'Quantity', 'Unit'].map(h => <p key={h} className="text-[9px] font-bold uppercase tracking-widest text-[#a020f0]">{h}</p>)}
                            </div>
                            {items.map((item, i) => (
