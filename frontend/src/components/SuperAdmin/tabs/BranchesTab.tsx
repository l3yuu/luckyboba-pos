...
const DeleteBranchModal: React.FC<{
  ...
  <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
    <div className="bg-white rounded-[2rem] shadow-2xl p-8 w-full max-w-sm text-center">
      ...
      <h3 className="font-black text-[#3b2063] text-lg uppercase tracking-tight mb-2">Delete Branch?</h3>
      <p className="text-zinc-400 text-sm mb-6">
        Are you sure you want to delete{' '}
        <span className="font-black text-[#3b2063]">{branch.name}</span>?
        This action cannot be undone.
      </p>
      <div className="flex gap-3">
        <button onClick={onCancel} disabled={loading}
          className="flex-1 py-3 rounded-2xl border border-zinc-200 text-xs font-black uppercase text-zinc-500 hover:bg-zinc-50 disabled:opacity-50">
          Cancel
        </button>
        <button onClick={onConfirm} disabled={loading}
          className="flex-1 py-3 rounded-2xl bg-red-500 text-white text-xs font-black uppercase hover:bg-red-600 disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        <div className="w-full relative" style={{ height: '240px' }}>
          ...
          <div className="absolute left-14 right-0 top-0 bottom-8">
            <svg viewBox={`0 0 ${chartW} ${chartH}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
              ...
              {hoveredIndex !== null && (
                <line
                  x1={(hoveredIndex / Math.max(weeklyNorm.length - 1, 1)) * chartW} y1="0"
                  x2={(hoveredIndex / Math.max(weeklyNorm.length - 1, 1)) * chartW} y2={chartH}
                  stroke="#3b2063" strokeWidth="1" strokeDasharray="4 3" opacity="0.3"
                />
              )}
              {weeklyNorm.length > 1 && (
                <polygon points={`0,${chartH} ${weeklyPoints} ${chartW},${chartH}`} fill="url(#weekGrad)" opacity="0.15" />
              )}
              {weeklyNorm.length > 1 && (
                <polyline points={weeklyPoints} fill="none" stroke="#3b2063" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" />
              )}
              {weeklyNorm.map((y, i) => {
                ...
                return (
                  <g key={i}>
                    ...
                    <circle cx={cx} cy={cy} r={isHovered ? 7 : 5}
                      fill={isHovered ? '#3b2063' : 'white'} stroke="#3b2063" strokeWidth="3"
                      style={{ transition: 'r 0.15s, fill 0.15s', pointerEvents: 'none' }} />
                    {isHovered && (
                      <g>
                        <rect x={cx - 52} y={cy - 44} width="104" height="32" rx="8" ry="8" fill="#3b2063" />
                        <polygon points={`${cx - 6},${cy - 13} ${cx + 6},${cy - 13} ${cx},${cy - 4}`} fill="#3b2063" />
                        <text x={cx} y={cy - 34} textAnchor="middle" fill="white" fontSize="9" fontWeight="800" fontFamily="system-ui" style={{ pointerEvents: 'none' }}>
                          {weeklyLabels[i]}
                        </text>
                        <text x={cx} y={cy - 20} textAnchor="middle" fill="#c4b5fd" fontSize="10" fontWeight="900" fontFamily="system-ui" style={{ pointerEvents: 'none' }}>
                          {fmt(weeklyTotals[i])}
                        </text>
                      </g>
                    )}
            ...
            <div className="flex justify-between mt-3 text-[9px] font-black text-zinc-300 uppercase tracking-widest">
              {weeklyLabels.map((d, i) => (
                <span key={i} className={`transition-colors ${hoveredIndex === i ? 'text-[#3b2063]' : ''}`}>{d}</span>
              ))}
            </div>
          </div>
        </div>
          ...
          {hourlyNorm.map((h, i) => {
            ...
            return (
              <div key={i} className="flex flex-col items-center min-w-[2.5rem] flex-1 h-full justify-end relative"
                ...
                {isHovered && hasData && (
                  <div className="absolute left-1/2 -translate-x-1/2 bg-[#3b2063] text-white rounded-xl px-3 py-2 whitespace-nowrap pointer-events-none z-20 shadow-lg"
                ...
                <span className="text-[8px] font-black uppercase whitespace-nowrap absolute bottom-0"
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled) { setAnalytics(data); setAnalyticsLoading(false); }
      } catch (e: unknown) {
        if (!cancelled) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 md:p-10">
      <div className="w-full max-w-7xl max-h-[95vh] flex flex-col bg-[#f8f6ff] rounded-[2.5rem] shadow-2xl overflow-hidden">
        <div className="flex justify-between items-center bg-white px-8 py-6 border-b border-zinc-100 shrink-0">
          <div>
            <h2 className="text-[#3b2063] font-black text-2xl uppercase tracking-tighter">{branch.name} Analytics</h2>
            <p className="text-zinc-400 text-xs font-bold uppercase tracking-[0.2em] mt-1">{branch.location}</p>
          </div>
          <div className="flex items-center gap-4">
            <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${branch.status === 'active' ? 'bg-emerald-100 text-emerald-600' : 'bg-zinc-100 text-zinc-400'}`}>
              {branch.status}
            </span>
            <button onClick={onClose} className="p-3 bg-zinc-100 rounded-full hover:bg-zinc-200 transition-colors text-zinc-500">
...
const BranchesTab: React.FC<BranchesTabProps> = ({
  ...
  const handleDeleteClick = (branch: Branch) => setBranchToDelete(branch);
              ...
              <div key={branch.id} className="rounded-[2.5rem] border border-zinc-100 bg-white shadow-sm p-8 transition-all hover:shadow-md flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-8 py-6 border-y border-zinc-50 flex-1">
                  <div>
                    ...
                    <p className="font-black text-xl text-[#3b2063]">₱{(parseFloat(String(branch.total_sales)) || 0).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-zinc-300 mb-1">Today's Revenue</p>
                    <p className="font-black text-xl text-emerald-500">₱{(parseFloat(String(branch.today_sales)) || 0).toLocaleString()}</p>
                  </div>
                </div>
                <div className="flex gap-3 mt-8">
                  ...
                  <button onClick={() => handleDeleteClick(branch)} disabled={loading}
