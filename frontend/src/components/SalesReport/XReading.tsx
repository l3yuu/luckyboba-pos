...
interface XReadingReport {
  ...
  hourly_data: { time: string; total: number; count: number }[];
}
...
const XReading = () => {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const response = await api.get('/reports/x-reading', {
        params: { date: selectedDate }
      });
      setReportData(response.data);
    } catch (error) {
      console.error("X-Reading fetch failed:", error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col relative">
        ...
        <div className="bg-white p-3 rounded-lg shadow-sm border border-zinc-200 mb-6 flex flex-col xl:flex-row items-center gap-4 relative z-50 print:hidden">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {menuCards.map((card, index) => (
                    <div 
                      key={index}
                      onClick={() => { 
                        if(card.actionLabel === "X-READING") handleGenerate(); 
                        setIsMenuOpen(false); 
                      }} 
          ...
          <div className="flex gap-2 w-full xl:w-auto ml-auto pl-4 border-l border-zinc-200">
            ...
            <button 
              onClick={handlePrint} 
              className="px-6 h-12 bg-[#172554] text-white rounded-md font-bold uppercase text-xs tracking-wider hover:bg-[#0f172a] transition-all flex items-center justify-center gap-2 shadow-sm min-w-25"
            >
          ...
          {reportData ? (
            <div className="bg-white w-full max-w-150 p-10 shadow-2xl border border-zinc-100 font-mono text-slate-800">
              <div className="text-center mb-6">
                ...
                <p className="text-[10px] text-zinc-400">X-READING REPORT</p>
                <p className="text-[10px] mt-2">{selectedDate}</p>
              </div>

              <div className="border-t-2 border-dashed border-zinc-200 my-4 pt-4 space-y-2">
                <div className="flex justify-between text-xs"><span>GROSS SALES</span> <span className="font-bold">{phCurrency.format(reportData.gross_sales)}</span></div>
                <div className="flex justify-between text-xs"><span>NET SALES</span> <span className="font-bold">{phCurrency.format(reportData.net_sales)}</span></div>
              ...
              <div className="border-t border-dashed border-zinc-200 my-4 pt-4 space-y-2">
                <div className="flex justify-between text-xs"><span>CASH SALES</span> <span>{phCurrency.format(reportData.cash_total)}</span></div>
                <div className="flex justify-between text-xs"><span>GCASH/OTHERS</span> <span>{phCurrency.format(reportData.non_cash_total)}</span></div>
              </div>

              <div className="mt-10 text-center">
                <p className="text-[9px] uppercase tracking-widest text-zinc-300">End of Report</p>
              </div>
            </div>
          ) : (
