    ...
    switch (type) {
      case 'cash_count': {
        ...
        if (flatDenominations) {
          return { ...data, cash_count: { denominations: flatDenominations, grand_total: flatGrandTotal ?? 0 } } as unknown as XReadingReport;
        }
    ...
    <div className="my-2 pt-2">
      ...
      {reportData?.logs?.length ? (
        <table className="w-full text-[10px]">
          <thead><tr className="border-b border-black"><th className="text-left w-[30%]">TIME</th><th className="text-left w-[45%]">REASON</th><th className="text-right w-[25%]">AMT</th></tr></thead>
          <tbody>
      ...
      <div className="my-2 pt-2 animate-in fade-in duration-300">
        ...
        {reportData.categories.map((cat, catIdx) => (
          <div key={catIdx} className="mb-4">
            <p className="text-[15px] uppercase font-bold bg-zinc-100 px-2 py-0.5 mb-1 border-b border-black">{cat.category_name}</p>
                ...
                {cat.products.map((item, i) => (
                  <React.Fragment key={i}>
                    ...
                    {item.add_ons && item.add_ons.length > 0 && (
                      <tr><td colSpan={3} className="pb-2 pl-2"><div className="border-l-2 border-zinc-400 pl-2">{item.add_ons.map((addon, aIdx) => (<div key={aIdx} className="text-[10px] flex justify-between"><span>+ {addon.name}</span><span className="text-black">x{addon.qty}</span></div>))}</div></td></tr>
                    )}
                  </React.Fragment>
                ))}
              </tbody>
            ...
            <div className="flex justify-between items-center pt-1 mt-1 border-t border-black">
              <span className="text-[10px] uppercase font-bold">Sub-total {cat.category_name}</span>
        ...
        {reportData.all_addons_summary && reportData.all_addons_summary.length > 0 && (
          <div className="mt-4 mb-2 pt-2 border-t border-dashed border-black">
            ...
            <table className="w-full text-[11px]"><tbody>{reportData.all_addons_summary.map((addon, idx) => (<tr key={idx} className="border-b border-dotted border-zinc-300"><td className="py-0.5 uppercase font-semibold">{addon.name}</td><td className="py-0.5 text-right font-bold">x{addon.qty}</td></tr>))}</tbody></table>
          </div>
        )}
        <div className="receipt-divider"></div>
        <div className="space-y-1">
          <div className="flex-between text-[11px] font-bold"><span>TOTAL ITEMS SOLD</span><span>{reportData.categories.reduce((acc, cat) => acc + cat.products.reduce((pAcc, p) => pAcc + p.total_qty, 0), 0)}</span></div>
          ...
          <div className="flex-between text-[13px] font-black mt-2 pt-1 border-t border-black"><span>TOTAL REVENUE</span><span>{phCurrency.format(reportData?.grand_total_revenue || 0)}</span></div>
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="my-2 pt-2">
        ...
        {!denominations || denominations.length === 0 ? (
          ...
          <table className="w-full text-[11px]"><tbody>{denominations.map((d, i) => (<tr key={i} className="border-b border-dotted border-zinc-300"><td className="py-1 font-semibold">{d.label}</td><td className="py-1 text-center">x{d.qty}</td><td className="py-1 text-right font-bold">{phCurrency.format(d.total)}</td></tr>))}</tbody></table>
        )}
        <div className="receipt-divider"></div>
        <div className="flex-between text-[14px] font-black"><span>GRAND TOTAL</span><span>{phCurrency.format(grandTotal)}</span></div>
      </div>
    );
  };
  ...
  const renderDetailedSales = () => {
    ...
    return (
      <div className="my-2 pt-2">
            ...
            {rows.map((tx, i) => {
              ...
              return (
                <div key={i} className="border-b border-dashed border-zinc-300 pb-2 mb-2 last:border-0 text-[10px]">
                  <div className="flex-between font-black text-[12px] mb-0.5"><span className="truncate pr-2">#{tx.Invoice}</span><span>{phCurrency.format(tx.Amount)}</span></div>
                  <div className="flex-between text-[10px] font-bold"><span className="opacity-70">{dateTime}</span>{hasStatus && (<span className={`${String(status).toLowerCase() === 'completed' ? 'text-black' : 'text-red-600'} uppercase`}>{status}</span>)}</div>
                </div>
              );
            })}
          </div>
        ...
        <div className="receipt-divider"></div>
        <div className="flex-between text-[11px] font-bold mt-2"><span>TOTAL TRANSACTIONS</span><span>{rows.length}</span></div>
        <div className="flex-between text-[12px] mt-1 font-black"><span>TOTAL AMOUNT</span><span>{phCurrency.format(rows.reduce((acc, tx) => acc + Number(tx.Amount || 0), 0))}</span></div>
      </div>
    );
  };
      ...
      <div className="my-2 pt-2">
        ...
        {summaryRows.length === 0 ? (
          ...
          <table className="w-full text-[10px] mb-3">
            <thead><tr className="border-b border-black text-left"><th className="w-[45%] pb-1">DATE</th><th className="w-[20%] text-center pb-1">QTY</th><th className="w-[35%] text-right pb-1">AMOUNT</th></tr></thead>
            <tbody>{summaryRows.map((item, i) => (<tr key={i} className="border-b border-dotted border-zinc-300"><td className="py-1 font-semibold">{item.Sales_Date}</td><td className="py-1 text-center">{item.Total_Orders}</td><td className="py-1 text-right font-bold">{phCurrency.format(item.Daily_Revenue)}</td></tr>))}</tbody>
          </table>
        )}
        <div className="receipt-divider"></div>
        <div className="space-y-1 uppercase text-[10px] font-semibold">
          ...
          <div className="flex-between"><span>Gross Sales</span><span className="font-bold">{phCurrency.format(reportData?.gross_sales || 0)}</span></div>
          <div className="flex-between"><span>Total Discounts</span><span className="font-bold">-{phCurrency.format(reportData?.total_discounts || 0)}</span></div>
          <div className="flex-between"><span>Total Voids</span><span className="font-bold">{phCurrency.format(reportData?.total_void_amount || 0)}</span></div>
          <div className="receipt-divider"></div>
          <p className="text-[10px] font-bold opacity-80 mt-2 mb-1">Payment Methods</p>
          {reportData?.payment_breakdown?.map((p, i) => (<div key={i} className="flex-between text-[10px]"><span>{p.method}</span><span className="font-bold">{phCurrency.format(p.amount)}</span></div>))}
          <div className="receipt-divider"></div>
          ...
          <div className="flex-between text-[13px] font-black mt-3 pt-2 border-t border-black"><span>NET REVENUE</span><span>{phCurrency.format(reportData?.gross_sales || 0)}</span></div>
        </div>
      </div>
    );
  };
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col relative">
        <style>{`
          ...
          @media print {
            ...
            th { text-align: left !important; border-bottom: 2px solid #000 !important; padding-bottom: 4px !important; text-transform: uppercase !important; font-weight: 700 !important; font-size: 10px !important; word-wrap: break-word !important; overflow-wrap: break-word !important; }
            td { padding: 3px 0 !important; vertical-align: top !important; font-size: 10px !important; font-weight: 500 !important; word-wrap: break-word !important; overflow-wrap: break-word !important; }
          }
        `}</style>
        ...
        {showDebug && rawApiResponse && (
          <div className="mb-4 p-4 bg-zinc-900 text-green-400 text-[11px] font-mono overflow-auto max-h-64 print:hidden rounded-none">
            ...
            <div className="printable-receipt-container">
              <div className="receipt-area bg-white w-full max-w-[65mm] p-4 text-slate-800" style={{ fontFamily: "Arial, Helvetica, sans-serif" }}>
                <div className="text-center space-y-1">
                  <h1 className="uppercase leading-tight" style={{ fontWeight: 800, fontSize: '16px' }}>LUCKY BOBA MILKTEA<br/>FOOD AND BEVERAGE<br/>TRADING</h1>
                  <p className="uppercase" style={{ fontWeight: 600, fontSize: '11px', marginTop: '2px' }}>MAIN BRANCH – QC</p>
                  <div className="receipt-divider"></div>
                  <h2 className="uppercase tracking-widest" style={{ fontWeight: 900, fontSize: '14px', margin: '4px 0' }}>{reportData.report_type?.replace(/_/g, ' ') || 'REPORT'}</h2>
                  <div className="receipt-divider"></div>
