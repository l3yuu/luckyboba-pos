...
const SalesDashboard = () => {
  ...
  const dateRangeText = weekly.length > 0 ? `${weekly[0].date} — ${weekly[weekly.length - 1].date}, 2026` : 'No data available';
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 flex flex-col gap-4">
            ...
            <div className="flex-1 flex flex-col">
              <div className="relative" style={{ height: WEEKLY_HEIGHT }}>
                {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => (
                  <div key={i} className="absolute left-0 right-0 h-px bg-zinc-50" style={{ top: `${pct * 100}%` }} />
                ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-1">
          <div className="grid grid-cols-2 gap-3 content-start">
            <StatCard label="Beginning Sales" value="₱ 0.00" icon={<Banknote size={14}/>} />
            <StatCard label="Today Gross" value={fmt(stats.today_sales)} icon={<TrendingUp size={14}/>} isSuccess />
            <StatCard label="Live Ending" value={fmt(stats.today_sales)} icon={<Activity size={14}/>} isBrand />
            <StatCard label="Voided Journal" value={fmt(stats.cancelled_sales)} icon={<AlertCircle size={14}/>} isDanger />
            ...
            <div className="flex gap-4 flex-1" ref={barRef}>
              ...
              <div className="flex-1 flex flex-col">
                <div className="relative flex-1" style={{ height: TODAY_HEIGHT }}>
                  {yLabels.map((_, i) => (
                    <div key={i} className="absolute left-0 right-0 h-px bg-zinc-50" style={{ top: `${(i / (yLabels.length - 1)) * 100}%` }} />
                  ))}
                      ...
                      {filteredTodayData.map((d, i) => {
                        ...
                        return (
                          <div key={i} className="relative flex-1 h-full flex items-end group cursor-pointer"
                            ...
                            <div className="w-full rounded-none transition-all duration-700 ease-out"
                              style={{ height: `${pct}%`, background: isHovered ? '#7c3aed' : (d.value > 0 ? '#3b2063' : '#f8f6ff'), transitionDelay: `${i * 20}ms` }} />
                          </div>
                        );
                      })}
                    </div>
                ...
                <div className="flex gap-1 px-1 mt-4 border-t border-zinc-100 pt-3">
                  {filteredTodayData.map((d, i) => (
