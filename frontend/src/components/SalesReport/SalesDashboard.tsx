...
const SalesDashboard = () => {
  ...
  const { weekly, today: todayData, stats } = analytics;

  // ── WEEKLY GRAPH ──
  const WEEKLY_MAX = weekly.length > 0
    ? Math.max(...weekly.map((d: WeeklyDataPoint) => d.value), 10000)
    : 10000;

  const getLineY = (value: number) => WEEKLY_HEIGHT - (value / WEEKLY_MAX) * WEEKLY_HEIGHT;

  const linePoints: HoveredValuePoint[] = weekly.map((data: WeeklyDataPoint, index: number) => {
  ...
  const filteredTodayData = DISPLAY_HOURS.map(hour => {
    const match = todayData.find((d: TodayDataPoint) => parseHour(d.time) === hour);
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1 flex flex-col">
              <div className="relative" style={{ height: WEEKLY_HEIGHT }}>
                ...
                {linePoints.map((p: HoveredValuePoint, i: number) => (
                  <div
                    key={i}
              ...
              <div className="flex justify-between pt-3 mt-1 border-t border-zinc-50">
                {weekly.map((d: WeeklyDataPoint, i: number) => (
