...
const MallAccredReport = () => {
  ...
  const [reportData, setReportData] = useState<MallReportData | null>(null);
  ...
  const handleGenerate = useCallback(async (mallName?: string) => {
    // If mallName is passed (from menu), use it. Otherwise use state.
    const targetMall = mallName || selectedMall;
    if (!targetMall) {
        alert("Please select a mall from the MENU first.");
        return;
    }

    setLoading(true);
    try {
      const response = await api.get('/reports/mall-accreditation', {
        params: { 
            date: selectedDate, 
            mall: targetMall 
        }
      ...
      setSelectedMall(targetMall);
      setIsMenuOpen(false);
    } catch (error) {
      console.error("Failed to generate mall report:", error);
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col relative">
        <div className="bg-white p-3 rounded-lg shadow-sm border border-zinc-200 mb-6 flex flex-col xl:flex-row items-center gap-4 relative z-50 print:hidden">
          <div className="relative" ref={menuRef}>
            ...
            {isMenuOpen && (
              <div className="absolute top-full left-0 mt-2 w-225 bg-white rounded-xl shadow-2xl border border-zinc-200 p-6 animate-in fade-in slide-in-from-top-2 duration-200 z-50 max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-3 gap-4">
                  {malls.map((mall, index) => (
                    <div 
                      key={index}
                      onClick={() => handleGenerate(mall)}
                      className="bg-white border-l-4 border-blue-600 rounded-r-lg shadow-sm hover:shadow-md border-y border-r p-5 h-28 flex flex-col justify-center relative overflow-hidden group transition-all cursor-pointer"
                    >
        ...
        <div className="flex-1 flex flex-col items-center justify-start py-10">
          {reportData ? (
            <div className="bg-white w-full max-w-125 p-10 shadow-2xl border border-zinc-200 font-mono text-slate-800 animate-in fade-in zoom-in-95 duration-300">
                <div className="text-center border-b-2 pb-6 mb-6 border-slate-800">
                    <h1 className="text-2xl font-black">MALL SALES REPORT</h1>
                    <p className="text-blue-600 font-bold tracking-widest mt-1">{reportData.mall}</p>
                </div>
                
                <div className="space-y-4">
                    <div className="flex justify-between text-xs"><span>TENANT ID:</span> <span className="font-bold">{reportData.tenant_id}</span></div>
                    <div className="flex justify-between text-xs"><span>REPORT DATE:</span> <span className="font-bold">{reportData.date}</span></div>
                    
                    <div className="border-t border-dashed pt-4 flex justify-between text-lg font-black text-blue-700">
                        <span>GROSS SALES:</span>
                        <span>{phCurrency.format(reportData.gross_sales)}</span>
                    </div>
                    
                    <div className="flex justify-between text-xs text-zinc-500"><span>VAT (12%):</span> <span>{phCurrency.format(reportData.tax_amount)}</span></div>
                    <div className="flex justify-between text-xs text-zinc-500"><span>NET SALES:</span> <span>{phCurrency.format(reportData.net_sales)}</span></div>
