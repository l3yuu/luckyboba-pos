...
const STYLES = `
  ...
  .tl-stat-card:hover { border-color: #6a12b8; transform: translateY(-2px); box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.05); }
  ...
  .tl-search-input:focus { background: #fff; border-color: #6a12b8; outline: none; box-shadow: 0 0 0 4px rgba(59, 32, 99, 0.05); }
        ...
        <div className="flex items-center gap-2 p-1 bg-slate-100/80 border border-slate-200 rounded-2xl">
          {(['all', 'available', 'unavailable'] as const).map(f => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeFilter === f ? 'bg-[#6a12b8] text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
            >
              {f === 'all' ? 'All' : f}
            </button>
          ))}
                ...
                <tr key={item.id} className="hover:bg-slate-100/30 transition-colors group">
                  ...
                  <td className="px-6 py-5 text-right">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-4 py-2 border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-[#6a12b8] hover:border-[#6a12b8] transition-all group-hover:bg-white"
                    >
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 backdrop-blur-md bg-slate-900/40 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl border border-white overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8 pb-6 border-b border-slate-50 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-[#f5f0ff] rounded-2xl flex items-center justify-center text-[#6a12b8] shadow-sm shrink-0">
                  ...
                  {recipeData.map(recipe => (
                    <div key={recipe.id} className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 bg-[#6a12b810] text-[#6a12b8] rounded-lg">
