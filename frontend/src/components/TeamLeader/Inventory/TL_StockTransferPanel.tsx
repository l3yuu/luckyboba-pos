...
const inputCls = (err?: string) =>
  `w-full text-sm font-bold text-zinc-700 bg-zinc-50 border rounded-lg px-3 py-2.5 outline-none focus:ring-2 focus:ring-violet-400 focus:bg-white transition-all ${err ? 'border-red-300 bg-red-50' : 'border-zinc-200'}`;
...
const CreateTransferModal: React.FC<{
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 bg-zinc-900/40 backdrop-blur-sm transition-all animate-in fade-in duration-200">
      ...
      <div className="relative bg-white w-full max-w-2xl border border-zinc-200 rounded-[1.25rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200" style={dashboardFont}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-100 bg-zinc-50/50 shrink-0">
          ...
          <button onClick={onClose} className="p-2 hover:bg-zinc-100 rounded-lg text-zinc-300 transition-colors"><X size={18} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-[1fr_40px_1fr] items-center gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 px-0.5">Destination</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 px-0.5">Transfer Date</label>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 px-0.5">Notes</label>
...
const ViewTransferModal: React.FC<{
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 bg-zinc-900/40 backdrop-blur-sm transition-all animate-in fade-in duration-200">
      <div className="absolute inset-0" onClick={onClose} />
      <div className="relative bg-white w-full max-w-md border border-zinc-200 rounded-[1.25rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200" style={dashboardFont}>
...
const TL_StockTransferPanel: React.FC<{ branchId?: number | null }> = ({ branchId: propsBranchId }) => {
  const auth = useContext(AuthContext);
  const currentBranchId = propsBranchId || Number(localStorage.getItem('lucky_boba_user_branch_id') || auth?.user?.branch_id || 0);
  ...
  const filtered = useMemo(() => {
    const list = transfers.filter(t => {
      ...
      if (t.from_branch_id !== currentBranchId && t.to_branch_id !== currentBranchId) return false;
  ...
  }, [transfers, search, statusFilter, sortKey, sortDir, currentBranchId]);

  const stats = useMemo(() => {
    const counts = { Pending: 0, Approved: 0, 'In Transit': 0, Received: 0, Cancelled: 0 };
    transfers.filter(t => t.from_branch_id === currentBranchId || t.to_branch_id === currentBranchId).forEach(t => { 
        ...
        <div className="bg-white border border-zinc-200 rounded-[0.625rem] flex flex-col shadow-sm flex-1 overflow-hidden min-h-[400px]">
            ...
            <div className="px-6 py-4 border-b border-zinc-100 flex flex-wrap items-center gap-4">
                ...
                <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                    className="text-[10px] font-black uppercase tracking-widest text-zinc-500 bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-2 outline-none focus:ring-2 focus:ring-violet-400 cursor-pointer h-10 shadow-sm transition-all focus:bg-white appearance-none">
            ...
            <div className="flex-1 overflow-x-auto">
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-zinc-50/30 border-b border-zinc-100 uppercase tracking-tighter">
                    ...
                    <tbody className="divide-y divide-zinc-50">
                        ...
                        <tr><td colSpan={7} className="py-24 text-center">
                            ...
                            <p className="text-[11px] font-black text-zinc-300 uppercase tracking-widest italic">No transfers involving this branch found</p>
                        </td></tr>
                        ) : filtered.map(t => {
                        ...
                        return (
                            ...
                            <tr className={`hover:bg-[#faf9ff] transition-all group ${isExp ? 'bg-[#faf9ff]' : ''}`}>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="font-black text-[#1a0f2e] text-xs font-mono">{t.transfer_number}</span>
                                </div>
                                </td>
                                <td className="px-5 py-4">
                                    <span className="text-xs font-bold text-zinc-600 truncate block max-w-[120px]">{t.from_branch?.name ?? t.from_branch_name ?? '—'}</span>
                                </td>
                                <td className="px-5 py-4">
                                    <span className="text-xs font-black text-[#1a0f2e] truncate block max-w-[120px]">{t.to_branch?.name ?? t.to_branch_name ?? '—'}</span>
                                </td>
                                <td className="px-5 py-4">
                                ...
                                <td className="px-5 py-4">
                                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black text-white bg-[#a020f0] uppercase tracking-widest">
                                    {items.length} units
                                </span>
                                </td>
                                ...
                                <td className="px-7 py-4 text-right">
                                    ...
                                    <button onClick={() => setExpanded(isExp ? null : t.id)} className="p-1.5 hover:bg-white border hover:border-zinc-200 rounded-lg text-zinc-300 hover:text-[#a020f0] transition-all">
                                        <ChevronDown size={14} className={`transition-transform duration-300 ${isExp ? 'rotate-180' : ''}`} />
                                    </button>
                                    <button onClick={() => setViewTarget(t)} className="h-8 px-3 bg-white border border-zinc-200 rounded-lg text-zinc-500 hover:border-[#a020f0] hover:text-[#a020f0] font-black text-[9px] uppercase tracking-widest transition-all shadow-sm">
                                        View
                                    </button>
                                    </div>
                                </td>
                            ...
                            {isExp && (
                                <tr className="bg-[#faf9ff] border-b border-zinc-100 animate-in slide-in-from-top-1 duration-200">
                                <td colSpan={7} className="px-10 pb-5 pt-1">
                                    <div className="bg-white border border-[#e9d5ff] rounded-2xl p-5 shadow-sm max-w-xl">
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            {items.map((item, i) => (
                                                <div key={i} className="flex flex-col gap-0.5 p-3 bg-[#faf9ff] border border-zinc-100 rounded-xl">
                                                <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-tighter truncate">{item.material_name}</p>
                                                <div className="flex items-center justify-between">
                                                    <span className="text-sm font-black text-[#1a0f2e] tabular-nums">{item.quantity}</span>
                                                    <span className="text-[8px] font-black text-violet-600 bg-white px-2 py-0.5 rounded-lg border border-violet-50 uppercase tracking-widest">{item.unit}</span>
                                                </div>
                                                </div>
                                            ))}
                                        </div>
...
export default TL_StockTransferPanel;
