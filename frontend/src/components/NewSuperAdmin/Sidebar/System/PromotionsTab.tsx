...
import {
  ...
  Shuffle, Calendar, AlertTriangle,
} from "lucide-react";
...
const DiscountCard: React.FC<{
  ...
  return (
    <div className={`bg-white rounded-xl overflow-hidden border transition-all duration-200 ${
      deleting ? "opacity-50 scale-[0.98]" : "hover:shadow-md hover:-translate-y-0.5"
    ...
    <div className="p-6 md:p-8 fade-in">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <Btn onClick={() => setModalTarget("create")}><Plus size={13} /> Create Discount</Btn>
        </div>
      </div>
      ...
      {!loading && discounts.length === 0 && !error && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm font-bold text-zinc-600">No discounts yet</p>
          <p className="text-xs text-zinc-400 mt-1 mb-5">Create your first discount to get started.</p>
          <Btn onClick={() => setModalTarget("create")}><Plus size={13} /> Create Discount</Btn>
        </div>
      )}
