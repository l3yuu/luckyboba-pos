      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex items-center gap-5">
            <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-[#7c3aed] to-[#a020f0] flex items-center justify-center shrink-0 shadow-xl shadow-violet-200/50">
              ...
              {loading ? (
                ...
                <tr key={c.id} className="hover:bg-violet-50/20 transition-colors group cursor-pointer" onClick={() => setSelectedId(c.id)}>
                  <td className="px-4 py-1.5">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-white border border-zinc-200 flex items-center justify-center text-[10px] font-black text-[#a020f0] group-hover:border-violet-300 transition-colors">
                        {initials(c.name)}
                      </div>
                      <div className="min-w-0">
              ...
              {Array.from({ length: Math.min(5, meta.last_page) }, (_, i) => {
                ...
                return (
                  <button key={p} onClick={() => handlePage(p)}
                    className={`w-7 h-7 text-[10px] font-black rounded-lg transition-all border ${p === page ? "bg-[#a020f0] text-white border-[#a020f0]" : "text-zinc-500 bg-white border-zinc-200 hover:border-violet-300 hover:text-violet-600"}`}>
                    {p}
                  </button>
                );
              })}
