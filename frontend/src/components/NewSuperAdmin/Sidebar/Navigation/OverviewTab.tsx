...
const OverviewTab: React.FC = () => {
  ...
  const PIE_COLORS = ["#a020f0", "#6d3fa8", "#9b6bd4", "#c4a8e8", "#ddd0f8"];
    ...
    <div className="p-6 md:p-8 flex flex-col gap-6 fade-in pb-20">
      ...
      <div className="flex flex-wrap items-center justify-start gap-3 mb-6">
        ...
        <div className="text-left hidden sm:block">
          ...
          <p className="text-[0.7rem] font-black text-[#a020f0] tabular-nums leading-none mt-1">
        ...
        <div className="flex items-center gap-2">
          {(["daily", "weekly", "monthly"] as const).map(p => (
            <button key={p} onClick={() => setPeriod(p)} disabled={loading}
              className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all capitalize disabled:opacity-50 ${period === p ? "bg-[#a020f0] text-white" : "bg-white border border-zinc-200 text-zinc-600 hover:bg-zinc-50"}`}>
              {p}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        <div className="xl:col-span-8 bg-white border border-zinc-200 rounded-[0.625rem] p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <ShoppingBag size={16} className="text-[#a020f0]" />
              <h3 className="text-sm font-black text-[#1a0f2e] uppercase tracking-wider opacity-80">Live Sales Ticker</h3>
            </div>
            <span className="text-[10px] font-black text-[#a020f0] uppercase bg-[#ede8ff] px-2 py-0.5 rounded-md">
        ...
        <div className="xl:col-span-4 bg-white border border-zinc-200 rounded-[0.625rem] p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            <Activity size={16} className="text-[#a020f0]" />
            ...
            <div className="h-[220px] w-full min-w-0 relative">
              <ResponsiveContainer width="100%" height="100%" minHeight={100} minWidth={0} debounce={50}>
                <AreaChart data={revenueChartData}>
                  ...
                  />
                  <Area type="monotone" dataKey="revenue" stroke="#a020f0" strokeWidth={3} fill="url(#unifGrad)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
              ...
              <div className="mt-4 space-y-1.5 max-h-[80px] overflow-y-auto custom-scroll pr-1">
                {pieData.map((d, idx) => (
                  <div key={idx} className="flex items-center justify-between text-[11px]">
                    ...
                    <span className="font-black text-[#a020f0] whitespace-nowrap">{d.value}%</span>
                  </div>
                ))}
              </div>
            </>
            ...
            {loading && !barData.length ? <ChartSkeleton height="h-[200px]" /> : (
              <ResponsiveContainer width="100%" height="100%" minHeight={100} minWidth={0} debounce={50}>
                <ReBarChart data={barData} barSize={20}>
                  ...
                  <YAxis tick={{ fontSize: 10, fontWeight: 700, fill: "#a1a1aa" }} axisLine={false} tickLine={false} tickFormatter={fmtK} />
                  <Bar dataKey="sales" fill="#a020f0" radius={[4, 4, 0, 0]} />
                ...
                return (
                  <div key={i} className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-[0.625rem] bg-[#f5f0ff] border border-[#e9d5ff] flex items-center justify-center shrink-0">
                      <span className="text-[10px] font-black text-[#a020f0]">{i + 1}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="w-full h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                        <div className="h-full bg-[#a020f0] rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                    <span className="text-[10px] font-black text-[#a020f0] shrink-0 w-20 text-right">₱{Number(p.total_revenue).toLocaleString()}</span>
                  </div>
                );
              })
            )}
