...
const BRANCH_COLORS = ["#a020f0", "#6d3fa8", "#9b6bd4", "#c4a8e8", "#ddd0f8"];
const HOUR_COLORS = (hour: number) => {
  if (hour >= 11 && hour <= 14) return "#a020f0"; // lunch peak
        ...
        {loading ? <SkeletonBar h="h-[260px]" /> : revenue.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={revenue}>
              ...
              <Line yAxisId="rev" type="monotone" dataKey="revenue" name="Revenue" stroke="#a020f0" strokeWidth={2.5} dot={false} activeDot={{ r: 5 }} />
          ...
          {loading ? <SkeletonBar h="h-[280px]" /> : topProducts.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={topProducts} layout="vertical" barSize={14}>
                ...
                <Legend wrapperStyle={{ fontSize: 11, fontWeight: 600 }} />
                <Bar dataKey="total_quantity" name="Qty Sold" fill="#a020f0" radius={[0, 4, 4, 0]}>
                  {topProducts.map((_, i) => (
                    <Cell key={i} fill={i === 0 ? "#a020f0" : i === 1 ? "#6d3fa8" : i === 2 ? "#9b6bd4" : "#ede8ff"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="col-span-12 lg:col-span-5 bg-white border border-zinc-200 rounded-[0.625rem] p-6">
          ...
          <div className="flex items-center gap-4 mt-3">
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-sm bg-[#a020f0]" />
              ...
              {branchPerf.map((b, i) => {
                ...
                return (
                  <div key={b.branch_id} className={`rounded-[0.625rem] border px-4 py-3.5 ${isTop ? "bg-emerald-50 border-emerald-200" :
                    ...
                    <p className="text-base font-black text-[#a020f0] tabular-nums">{fmt(Number(b.total_revenue))}</p>
