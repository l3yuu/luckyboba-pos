...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const [isPosDropdownOpen, setPosDropdownOpen] = useState(false);


  useEffect(() => {
    const posIds = ['cash-in', 'cash-drop', 'menu', 'search-receipts', 'cash-count'];
    if (posIds.includes(currentTab)) {
      ...
      `}>
        <div className="px-6 pt-10 flex flex-col items-center overflow-y-auto no-scrollbar">
          ...
          <img 
            src={logo} 
            alt="Lucky Boba Logo" 
            className="w-55 h-auto object-contain mb-2 hidden md:block" 
          />
          <div className="text-[#3b2063] font-black uppercase text-[9px] tracking-[0.3em] opacity-60 mb-8">
            POS System
          </div>
          ...
          <nav className="w-full space-y-2">
            ...
            <button
              ...
              className={`w-full px-5 py-3 rounded-2xl font-black text-[13px] uppercase tracking-wider flex items-center transition-all duration-200 ${
                currentTab === 'dashboard' 
                  ...
                  : `text-zinc-400 ${hoverClasses}` // Updated Hover State
              }`}
            >
            ...
            <div className="w-full">
              <button
                ...
                className={`w-full px-5 py-3 rounded-2xl font-black text-[13px] uppercase tracking-wider flex items-center justify-between transition-all duration-200 ${
                  isPosDropdownOpen || posMenuItems.some(i => i.id === currentTab)
                    ? 'text-[#3b2063] bg-[#f0ebff]' // Updated Active/Open State
                    : `text-zinc-400 ${hoverClasses}` // Updated Hover State
                }`}
              >
                ...
                <svg 
                  ...
                  strokeWidth={2} 
                  stroke="currentColor" 
                  className={`w-3 h-3 transition-transform duration-300 ${isPosDropdownOpen ? 'rotate-180' : 'rotate-0'}`}
                >
              ...
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isPosDropdownOpen ? 'max-h-64 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                <div className="flex flex-col space-y-1 pl-4 border-l-2 border-[#f0ebff] ml-5"> {/* Updated border color */}
                  {posMenuItems.map((item) => (
                    <button
                      ...
                      className={`
                        text-left px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-colors
                        ${currentTab === item.id 
                          ? 'text-[#3b2063] bg-[#f0ebff]' 
                          : `text-zinc-400 ${hoverClasses}`} // Updated Hover State
                      `}
                    >
                      {item.label}
        ...
        <div className="px-8 pb-8 flex flex-col gap-6 bg-white pt-4 z-10">
          <button
            ...
            disabled={isLoading}
            className={`
              flex items-center justify-center w-full px-6 py-4 rounded-2xl
              bg-[#be2525] hover:bg-[#a11f1f] text-white 
              text-[11px] font-black uppercase tracking-[0.2em] 
              transition-all duration-200 shadow-md shadow-red-900/10
        ...
        <button 
          ...
          className="md:hidden absolute top-4 right-4 text-zinc-400 text-xs font-bold"
        >
          CLOSE
        </button>
      </aside>
