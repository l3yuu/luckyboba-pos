      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-6 bg-[#3b2063]/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-10 shadow-2xl border border-zinc-100 flex flex-col items-center text-center transition-all animate-in zoom-in-95 duration-200">
            ...
            <p className="text-zinc-500 text-sm font-medium mb-8 leading-relaxed px-2">Are you sure you want to log out of your current shift?</p>
            
            <div className="flex flex-col w-full gap-3">
              <button onClick={handleLogout} className="w-full py-4 bg-[#be2525] text-white rounded-2xl font-black uppercase tracking-widest text-[11px] hover:bg-[#a11f1f] transition-all active:scale-95 shadow-lg shadow-red-100">
                Yes, Logout
              </button>
              <button onClick={() => setShowLogoutConfirm(false)} className="w-full py-4 bg-white text-zinc-400 border border-zinc-100 rounded-2xl font-black uppercase tracking-widest text-[11px] hover:bg-zinc-50 transition-all active:scale-95">
                Cancel
              </button>
            </div>
          </div>
              ...
              <div className="w-full" key={dropdown.label}>
                <button
                  ...
                  className={`w-full px-5 py-3 rounded-2xl font-black text-[13px] uppercase tracking-wider flex items-center justify-between transition-all duration-200 ${
                    dropdown.state || dropdown.items.some(i => i.id === currentTab) ? 'text-[#3b2063] bg-[#f0ebff]' : `text-zinc-400 ${hoverClasses}`
                  }`}
                >
